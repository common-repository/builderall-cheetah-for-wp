...
YUI.add('ba-cheetah-slideshow', function(Y) {

/**
 * @module ba-cheetah-slideshow
 */
 ...
 * @extends Widget
 */
Y.namespace('BA').SlideshowCaption = Y.Base.create('ba-cheetah-slideshow-caption', Y.Widget, [Y.WidgetChild], {
	...
	renderUI: function()
	{
		...
		this._textToggleLink.addClass('ba-cheetah-slideshow-caption-toggle');
	 ...
	 * @static
	 */
	CSS_PREFIX: 'ba-cheetah-slideshow-caption',
 ...
 * @extends Widget
 */
Y.namespace('BA').SlideshowFrame = Y.Base.create('ba-cheetah-slideshow-frame', Y.Widget, [Y.WidgetParent, Y.WidgetChild], {
	...
	/**
	 * The active BA.SlideshowImage instance in the frame.
	 *
	 * @property _activeImage
	 * @type BA.SlideshowImage
	 * @default null
	 * @protected
	 */
	_activeImage: null,
	...
	/**
	 * A BA.SlideshowImage instance used to load the
	...
	/**
	 * An instance of BA.SlideshowTransition used for
	 * the current transition in progress.
	 *
	 * @property _transition
	 * @type BA.SlideshowTransition
	 * @default null
	 * @protected
	 */
	_transition: null,
	...
	/**
	 * Creates new instances of BA.SlideshowImage used in the frame.
	 *
	 * @method initializer
	 * @protected
	 */
	initializer: function()
	{
		...
		this._activeImage = new Y.BA.SlideshowImage(imageConfig);
		this._nextImage = new Y.BA.SlideshowImage(imageConfig);
	},

	/**
	 * Renders the BA.SlideshowImage instances used in the frame.
	 *
	 * @method renderUI
	 * @protected
	 */
	...
	bindUI: function()
	{
		...
		if(('ontouchstart' in window || window.navigator.msPointerEnabled) && this.get('touchSupport')) {
			...
			this._gestures = new Y.BA.SlideshowGestures({
	 ...
	 * using a new instance of BA.SlideshowTransition.
	 *
	...
	_transitionStart: function()
	{
		...
		this._transition = new Y.BA.SlideshowTransition({
	...
	_gesturesMoveStart: function(e)
	{
		...
		Y.BA.SlideshowImageLoader.removeGroup(this._nextImage.get('loadGroup'));
	 ...
	 * @static
	 */
	CSS_PREFIX: 'ba-cheetah-slideshow-frame',
	...
	ATTRS: {
		 ...
		 * BA.SlideshowImage. See the API docs for {@link BA.SlideshowImage}
 ...
 * @extends Plugin.Base
 */
Y.namespace('BA').SlideshowFullscreen = Y.Base.create('ba-cheetah-slideshow-fullscreen', Y.Plugin.Base, [], {
	...
	initializer: function()
	{
		...
		bb.addClass('ba-cheetah-fullscreen-enabled');

		if(Y.BA.SlideshowFullscreen.OS_SUPPORT) {
	...
	_browserEnter: function()
	{
		...
		Y.Node.one('body').on('ba-cheetah-fullscreen|keydown', Y.bind(this._onKey, this));
	...
	_browserExit: function()
	{
		...
		Y.Node.one('body').detach('ba-cheetah-fullscreen|keydown');
	...
	_enter: function()
	{
		...
		bb.addClass('ba-cheetah-fullscreen-active');
	...
	_exit: function()
	{
		...
		bb.removeClass('ba-cheetah-fullscreen-active');
	...
	{
		this._closeMessage = Y.Node.create('<div class="ba-cheetah-fullscreen-close-message"></div>');
 ...
 * @extends Base
 */
Y.namespace('BA').SlideshowGestures = Y.Base.create('ba-cheetah-slideshow-gestures', Y.Base, [], {
 ...
 * @static
 */
Y.namespace('BA').SlideshowImageLoader = {
 ...
 * @extends Widget
 */
Y.namespace('BA').SlideshowImage = Y.Base.create('ba-cheetah-slideshow-image', Y.Widget, [Y.WidgetChild], {
	...
	syncUI: function()
	{
		...
		if(this.get('crop')) {
			...
			bb.addClass('ba-cheetah-slideshow-image-cropped');
		}
	},
	...
	_loadImageComplete: function(img)
	{
		...
		this._image.addClass('ba-cheetah-slideshow-image-img');
	...
	_loadVideo: function()
	{
		...
		// Insert the video
		if(this._videoBox) {
			this._videoBox.one('.ba-cheetah-slideshow-video-wrap').insert(this._video);
		}
		else {
			bb.all('img').remove();
	...
	_insertVideoButton: function()
	{
		...
		this._videoButton = Y.Node.create('<a class="ba-cheetah-slideshow-video-button" href="javascript:void(0);"></a>');
	...
	{
		var root 	= this.get('root'),
			wrap 	= Y.Node.create('<div class="ba-cheetah-slideshow-video-wrap"></div>'),
			close 	= Y.Node.create('<a class="ba-cheetah-slideshow-video-close" href="javascript:void(0);"></a>'),
			event 	= 'ontouchstart' in window ? 'touchstart' : 'click';

		this._videoBox = Y.Node.create('<div class="ba-cheetah-slideshow-video"></div>');
		...
		Y.one('body').on('ba-cheetah-slideshow-image|keydown', this._onKey, this);
	},
	...
	_removeVideoBox: function(e)
	{
		if(typeof e !== 'undefined' && e.target) {
			if(e.target.get('className').indexOf('ba-cheetah-slideshow-video') < 0) {
				return;
			}
		}
		...
		Y.one('body').detach('ba-cheetah-slideshow-image|keydown', this._onKey);
	},
	 ...
	 * @static
	 */
	CSS_PREFIX: 'ba-cheetah-slideshow-image',
 ...
 * @extends Plugin.Base
 */
Y.namespace('BA').SlideshowMouseNav = Y.Base.create('ba-cheetah-slideshow-mouse-nav', Y.Plugin.Base, [], {
    ...
    _showArrow: function(e)
    {
    	...
    	if(host.albumInfo !== null && host.albumInfo.images.length > 1) {
        	if(layerX >= triggerWidth/2) {
        		trigger.removeClass('ba-cheetah-slideshow-mouse-nav-prev');
        		trigger.addClass('ba-cheetah-slideshow-mouse-nav-next');
        	}
        	else {
        		trigger.removeClass('ba-cheetah-slideshow-mouse-nav-next');
        		trigger.addClass('ba-cheetah-slideshow-mouse-nav-prev');
        	}
        }
    },
 ...
 * @extends Base
 */
Y.namespace('BA').SlideshowKenBurns = Y.Base.create('ba-cheetah-slideshow-ken-burns', Y.Base, [], {
	...
	run: function()
	{
		...
		if(Y.BA.Utils.cssSupport('transform')) {
	...
	_getTransform: function()
	{
        ...
        i = Math.floor(Math.random() * Y.BA.SlideshowKenBurns.ZOOM_DIRECTIONS.length);
        zoomDirection = Y.BA.SlideshowKenBurns.ZOOM_DIRECTIONS[i];

        // Random transform
        i = Math.floor(Math.random() * Y.BA.SlideshowKenBurns.TRANSFORMS.length);
        transform = Y.BA.SlideshowKenBurns.TRANSFORMS[i];

        // Get the start and end transforms
        if(!image.hasClass('ba-cheetah-slideshow-image-cropped') && zoomDirection == 'in') {
	...
	ATTRS: {
		...
		/**
		 * An instance of BA.Slideshow image to apply the
		 * Ken Burns effect on.
		 *
		 * @attribute image
		 * @type BA.Slideshow
 ...
 * @extends Widget
 */
Y.namespace('BA').SlideshowNav = Y.Base.create('ba-cheetah-slideshow-nav', Y.Widget, [Y.WidgetChild], {
	...
	_renderButtons: function()
	{
		...
		for( ; i < b.length; i++) {
			for(k = 0; k < b[i].names.length; k++) {
				...
				this._buttons[name].addClass('ba-cheetah-slideshow-nav-' + name);
	...
	{
		var html 		= '',
			countText	= Y.BA.SlideshowNav.COUNT_TEXT,
			current 	= 1,
			total 		= 1;
	...
	_updateCaption: function()
	{
		...
		if(imageInfo && imageInfo.caption === '') {
			...
			this._buttons.caption.addClass('ba-cheetah-slideshow-nav-caption-disabled');
		}
		else {
			this._buttons.caption.on('click', root._toggleCaption, root);
			this._buttons.caption.removeClass('ba-cheetah-slideshow-nav-caption-disabled');
		}
	},
	...
	{
		this._buttons.play.removeClass('ba-cheetah-slideshow-nav-pause');
		this._buttons.play.addClass('ba-cheetah-slideshow-nav-play');
	...
	{
		this._buttons.play.removeClass('ba-cheetah-slideshow-nav-play');
		this._buttons.play.addClass('ba-cheetah-slideshow-nav-pause');
	 ...
	 * @static
	 */
	CSS_PREFIX: 'ba-cheetah-slideshow-nav',
 ...
 * @extends Plugin.Base
 */
Y.namespace('BA').SlideshowOverlay = Y.Base.create('ba-cheetah-slideshow-overlay', Y.Plugin.Base, [], {
	...
	initializer: function()
	{
		...
		bb.addClass('ba-cheetah-slideshow-overlay');
    },
	...
	_initCloseButton: function()
	{
		...
		closeButton = Y.Node.create('<a class="ba-cheetah-slideshow-overlay-close" href="javascript:void(0);"></a>');
...
Y.namespace('BA').SlideshowSocial = Y.Base.create('ba-cheetah-slideshow-social', Y.Widget, [Y.WidgetChild], {
	 ...
	 * @static
	 */
	CSS_PREFIX: 'ba-cheetah-slideshow-social',
...
/**
 * Creates a grid of BA.SlideshowImage instances.
 *
 * @namespace BA
 ...
 * @extends Widget
 */
Y.namespace('BA').SlideshowThumbs = Y.Base.create('ba-cheetah-slideshow-thumbs', Y.Widget, [Y.WidgetParent, Y.WidgetChild], {
	 ...
	 * of BA.SlideshowImage instances.
	 *
	...
	/**
	 * An array of BA.SlideshowImage instances in the active page.
	 *
	...
	/**
	 * An array of BA.SlideshowImage instances used to
	...
	/**
	 * An instance of BA.SlideshowNav used for the left nav.
	 *
	 * @property _leftNav
	 * @type Object
	 * @default null
	...
	/**
	 * An instance of BA.SlideshowNav used for the right nav.
	 *
	 * @property _rightNav
	 * @type Object
	 * @default null
	...
	/**
	 * An instance of BA.SlideshowNav used for the top nav.
	 *
	 * @property _topNav
	 * @type Object
	 * @default null
	...
	/**
	 * An instance of BA.SlideshowNav used for the bottom nav.
	 *
	 * @property _bottomNav
	 * @type Object
	 * @default null
	...
	/**
	 * An instance of BA.SlideshowTransition for the current transition.
	 *
	 * @property _transition
	 * @type BA.SlideshowTransition
	 * @default null
	...
	destructor: function()
	{
		...
		Y.BA.SlideshowImageLoader.removeGroup('thumbs');
	},
	...
	_imageLoadComplete: function()
	{
		var albumInfo 	= this.get('root').albumInfo,
			lastActive 	= Y.one('.ba-cheetah-slideshow-image-active'),
		...
		nextActive = Y.one('.ba-cheetah-slideshow-image-active');
	...
	_renderBoxes: function()
	{
		...
		this._clipBox.addClass('ba-cheetah-slideshow-thumbs-clip');
		this.get('contentBox').insert(this._clipBox);

		// Pages box
		this._pagesBox = Y.Node.create('<div></div>');
		this._pagesBox.addClass('ba-cheetah-slideshow-thumbs-pages');
		this._clipBox.insert(this._pagesBox);

		// Active page box
		this._activePageBox = Y.Node.create('<div></div>');
		this._activePageBox.addClass('ba-cheetah-slideshow-thumbs-page');
		this._pagesBox.insert(this._activePageBox);

		// Next page box
		this._nextPageBox = Y.Node.create('<div></div>');
		this._nextPageBox.addClass('ba-cheetah-slideshow-thumbs-page');
	...
	_renderImage: function(imageArray, i, page, imageInfo)
	{
		...
		if(typeof imageArray[i] == 'undefined') {
			...
			imageArray[i] = new Y.BA.SlideshowImage(imageConfig);
		...
		for( ; i < imageArray.length; i++) {
			if(imageArray[i]._imageInfo) {
				if(imageArray[i]._imageInfo.index == this.get('root').imageInfo.index) {
					imageArray[i].get('boundingBox').addClass('ba-cheetah-slideshow-image-active');
					break;
				}
			}
		}
	...
	/**
	 * Removes the class name 'ba-cheetah-slideshow-image-active'
	...
	{
		var active = Y.one('.ba-cheetah-slideshow-image-active');

		if(active) {
			active.removeClass('ba-cheetah-slideshow-image-active');
		}
	},
	...
	_renderNavs: function()
	{
		...
		if(this.get('topNavEnabled') && topNavButtons.length > 0) {
			this._topNav = new Y.BA.SlideshowNav({ buttons: topNavButtons });
			this._topNav.get('boundingBox').addClass('ba-cheetah-slideshow-thumbs-top-nav');
		...
		if(this.get('rightNavEnabled') && rightNavButtons.length > 0) {
			this._rightNav = new Y.BA.SlideshowNav({ buttons: rightNavButtons });
			this._rightNav.get('boundingBox').addClass('ba-cheetah-slideshow-thumbs-right-nav');
		...
		if(this.get('bottomNavEnabled') && bottomNavButtons.length > 0) {
			this._bottomNav = new Y.BA.SlideshowNav({ buttons: bottomNavButtons });
			this._bottomNav.get('boundingBox').addClass('ba-cheetah-slideshow-thumbs-bottom-nav');
		...
		if(this.get('leftNavEnabled') && leftNavButtons.length > 0) {
			this._leftNav = new Y.BA.SlideshowNav({ buttons: leftNavButtons });
			this._leftNav.get('boundingBox').addClass('ba-cheetah-slideshow-thumbs-left-nav');
	...
	{
		var buttons = this.get('boundingBox').all('.ba-cheetah-slideshow-nav-prevPage, .ba-cheetah-slideshow-nav-nextPage'),
	 ...
	 * @static
	 */
	CSS_PREFIX: 'ba-cheetah-slideshow-thumbs',
 ...
 * @extends Base
 */
Y.namespace('BA').SlideshowTransition = Y.Base.create('ba-cheetah-slideshow-transition', Y.Base, [], {
	...
	initializer: function()
	{
		var type 		               = this.get('type'),
			typeArray	               = [],
			types                      = Y.BA.SlideshowTransition.TYPES,
			slideshowImageTypes        = Y.BA.SlideshowTransition.SLIDESHOW_IMAGE_TYPES,
		...
		if(Y.BA.SlideshowTransition.TYPES[type]) {
	 ...
	 * on an instance of BA.SlideshowImage or not.
	 *
	...
	_isSlideshowImage: function()
	{
        ...
        if(itemIn && itemIn.hasClass('ba-cheetah-slideshow-image')) {
			return true;
		}
		else if(itemOut && itemOut.hasClass('ba-cheetah-slideshow-image')) {
			return true;
		}
	...
	_transitionBars: function()
	{
        ...
        this.get('itemIn').one('.ba-cheetah-slideshow-image-img').setStyle('opacity', 0);
	...
	_renderSlices: function(numRows, numCols, multidimensional)
	{
        ...
        for( ; row < numRows; row++) {
            ...
            for(col = 0; col < numCols; col++) {
                ...
                slice = Y.Node.create('<div class="ba-cheetah-slideshow-transition-slice"></div>');
	...
	_transitionSlicesFadeLast: function(delay)
	{
        ...
        if(itemOut && !itemOut.hasClass('ba-cheetah-slideshow-image-cropped')) {
			itemOut.transition({
	...
	_transitionSlicesComplete: function()
	{
        ...
        itemIn.all('.ba-cheetah-slideshow-transition-slice').remove();
        itemIn.one('.ba-cheetah-slideshow-image-img').setStyle('opacity', 1);
        this._transitionComplete();
	},
	 ...
	 * run on BA.SlideshowImage widgets.
	 *
 ...
 * @constructor
 * @param config {Object} Configuration object
 * @extends BA.SlideshowBase
 */
Y.namespace('BA').Slideshow = Y.Base.create('ba-cheetah-slideshow', Y.BA.SlideshowBase, [], {

	/**
	 * A BA.SlideshowFrame instance used for the main image.
	 *
	 * @property frame
	 * @type BA.SlideshowFrame
	 * @default null
	 */
	frame: null,

	/**
	 * A BA.SlideshowNav instance used for the main nav.
	 *
	 * @property nav
	 * @type BA.SlideshowNav
	 * @default null
	 */
	nav: null,

	/**
	 * A BA.SlideshowNav instance used for the image nav's left button.
	 *
	...
	/**
	 * A BA.SlideshowNav instance used for the image nav's right button.
	 *
	...
	/**
	 * A BA.SlideshowThumbs instance used for the thumbnail grid.
	 *
	 * @property thumbs
	 * @type BA.SlideshowThumbs
	 * @default null
	 */
	thumbs: null,

	/**
	 * A BA.SlideshowThumbs instance used for the vertical thumbnail grid.
	 *
	...
	/**
	 * A BA.SlideshowCaption instance.
	 *
	 * @property caption
	 * @type BA.SlideshowCaption
	 * @default null
	 */
	caption: null,

	/**
	 * A BA.SlideshowSocial instance.
	 *
	 * @property social
	 * @type BA.SlideshowSocial
	 * @default null
	 */
	social: null,

	/**
	 * A BA.SlideshowImage instance used to preload
	 * the next image.
	 *
	...
	initializer: function()
	{
		...
		// Preloader
		this._nextImagePreloader = new Y.BA.SlideshowImage(imageConfig);
		...
		if(this._hasNavButton('fullscreen')) {
			if(Y.BA.SlideshowFullscreen.OS_SUPPORT) {
				this.plug(Y.BA.SlideshowFullscreen);
			}
			else {
				this._removeNavButton('fullscreen');
			}
	...
	/**
	 * Calls the BA.SlideshowBase superclass renderUI method
	...
	renderUI: function()
	{
		Y.BA.Slideshow.superclass.renderUI.apply(this, arguments);
	...
	/**
	 * Calls the BA.SlideshowBase superclass bindUI method, binds
	 * _resizeChildWidgets to fire after the resize method inherited
	 * from BA.SlideshowBase, shows the loading image, binds overlay events
	...
	bindUI: function()
	{
		...
		Y.BA.Slideshow.superclass.bindUI.apply(this, arguments);
		...
		ssBB.delegate('click', Y.bind(this._overlayCloseClick, this), '.ba-cheetah-slideshow-overlay-close');

		// Click action
		if(clickAction == 'gallery' || clickAction == 'url') {
            frameBB.delegate('click', Y.bind(this._frameClick, this), '.ba-cheetah-slideshow-image-img');
		}
	},

	/**
	 * Calls the BA.SlideshowBase superclass syncUI method
	...
	syncUI: function()
	{
		...
		Y.BA.Slideshow.superclass.syncUI.apply(this, arguments);
		...
		if(this.get('clickAction') != 'none') {
            this.frame.get('boundingBox').addClass('ba-cheetah-click-action-enabled');
		}
	},
	...
	_renderVerticalThumbs: function()
	{
		...
		if(this.get('verticalThumbsEnabled') && bbWidth > threshold) {
			...
			this.verticalThumbs = new Y.BA.SlideshowThumbs(this._getVerticalThumbsConfig());
			this.add(this.verticalThumbs);
			this.verticalThumbs.render(ssBB);

			vtBB = this.verticalThumbs.get('boundingBox');
			vtBB.addClass('ba-cheetah-slideshow-vertical-thumbs');
			vtBB.setStyle(this.get('verticalThumbsPosition'), 0);
			ssBB.append(vtBB);

			if(this.get('verticalThumbsOverlay')) {
				this.verticalThumbs.plug(Y.BA.SlideshowOverlay, {
	...
	/**
	 * Creates and renders a new instance of BA.SlideshowFrame
	...
	{
		this.frame = new Y.BA.SlideshowFrame({
			imageConfig: {
				loadGroup: 'main',
				loadPriority: true,
				crop: this.get('crop'),
		...
		this.frame.get('boundingBox').addClass('ba-cheetah-slideshow-main-image');
	...
	_loadFrame: function(e)
	{
		...
		Y.BA.SlideshowImageLoader.removeGroup('main-preload');
	...
	/**
	 * Creates and renders a new instance of BA.SlideshowNav
	...
	_renderNav: function()
	{
		...
		if(this.get('navType') == 'buttons') {
			...
			// Create the nav
			this.nav = new Y.BA.SlideshowNav({
				buttons: this.get('navButtons'),
				buttonsLeft: this.get('navButtonsLeft'),
				buttonsRight: this.get('navButtonsRight')
			});
			...
			// Plug overlay?
			if(navOverlay) {
				this.nav.plug(Y.BA.SlideshowOverlay, {
					hideDelay: this.get('overlayHideDelay')
				});
			...
			// CSS class name
			navBB.addClass('ba-cheetah-slideshow-main-nav');
		}
	},
	...
	/**
	 * Creates and renders two instances of BA.SlideshowNav for the
	...
	_renderImageNav: function()
	{
		...
		if(this.get('imageNavEnabled')) {
			if(this._isMobile()) {
				...
				this.imageNavLeft = new Y.BA.SlideshowNav({
				    buttons: ['prev'],
				    useFontIcons: false
				});

				this.imageNavRight = new Y.BA.SlideshowNav({
				    buttons: ['next'],
				    useFontIcons: false
				});
				...
				this.imageNavLeft.plug(Y.BA.SlideshowOverlay, { hideDelay: this.get('overlayHideDelay') });
				this.imageNavRight.plug(Y.BA.SlideshowOverlay, { hideDelay: this.get('overlayHideDelay') });
				...
				this.imageNavLeft.get('boundingBox').addClass('ba-cheetah-slideshow-image-nav-left');
				this.imageNavRight.get('boundingBox').addClass('ba-cheetah-slideshow-image-nav-right');
			}
		}
	},
	...
	_renderMouseNav: function()
	{
		if(this.get('mouseNavEnabled') && !('ontouchstart' in window) && !window.navigator.msPointerEnabled) {
			this.plug(Y.BA.SlideshowMouseNav, {
	...
	/**
	 * Creates and renders a new instance of BA.SlideshowThumbs.
	 *
	...
	_renderThumbs: function()
	{
		...
		if(this._thumbsEnabled()) {
			...
			// Create the thumbs
			this.thumbs = new Y.BA.SlideshowThumbs(this._getThumbsConfig());
			...
			if(navType == 'buttons' || navType == 'custom') {
				this.thumbs.plug(Y.BA.SlideshowOverlay, {
					hideDelay: this.get('overlayHideDelay'),
					hideStyle: 'left',
					visible: false
				});
	...
	/**
	 * Creates and renders a new instance of BA.SlideshowCaption.
	 *
	...
	_renderCaption: function()
	{
		if(this._hasNavButton('caption')) {
			...
			this.caption = new Y.BA.SlideshowCaption({
			...
			this.caption.plug(Y.BA.SlideshowOverlay, {
				hideDelay: this.get('overlayHideDelay'),
				visible: false,
				closeButton: true
			});
	...
	/**
	 * Creates and renders a new instance of BA.SlideshowSocial.
	 *
	...
	{
		if(this._hasNavButton('social')) {
			this.social = new Y.BA.SlideshowSocial();
			this.add(this.social);

			this.social.plug(Y.BA.SlideshowOverlay, {
				hideDelay: this.get('overlayHideDelay'),
				visible: false,
				closeButton: true
			});
	 ...
	 * @static
	 */
	CSS_PREFIX: 'ba-cheetah-slideshow',
...
}, '2.0.0' ,{requires:['anim', 'event-mouseenter', 'plugin', 'transition', 'ba-cheetah-event-move', 'ba-cheetah-slideshow-css', 'ba-cheetah-slideshow-base', 'ba-cheetah-utils', 'sm-fonticon']});


YUI.add('ba-cheetah-slideshow-album-loader', function(Y) {

/**
 * @module ba-cheetah-slideshow-album-loader
 */
 ...
 * @extends Base
 */
Y.namespace('BA').SlideshowAlbumLoader = Y.Base.create('ba-cheetah-slideshow-album-loader', Y.Base, [], {
	...
	*/
	...
	load: function(source)
	{
		...
		this[Y.BA.SlideshowAlbumLoader.TYPES[source.type]].call(this);
	},
...
}, '2.0.0' ,{requires:['base', 'ba-cheetah-smugmug-api']});


YUI.add('ba-cheetah-slideshow-base', function(Y) {

/**
 * @module ba-cheetah-slideshow-base
 */
 ...
 * @extends Widget
 */
Y.namespace('BA').SlideshowBase = Y.Base.create('ba-cheetah-slideshow-base', Y.Widget, [Y.WidgetParent], {

	/**
	 * BA.SlideshowAlbumLoader instance used to load albums.
	 *
	 * @property _loader
	 * @type BA.SlideshowAlbumLoader
	 * @default null
	 * @protected
	 */
	_albumLoader: null,
	...
	*/
	...
	/**
	 * An instance of BA.Spinner that is shown and hidden
	...
	{
		// Loader
		this._albumLoader = new Y.BA.SlideshowAlbumLoader({
	...
	bindUI: function()
	{
		...
		// Resize Events
		Y.one(window).on('ba-cheetah-slideshow-base|resize', this._delayResize, this);
		Y.one(window).on('ba-cheetah-slideshow-base|orientationchange', this._delayResize, this);
	...
	syncUI: function()
	{
		this.get('boundingBox').addClass('ba-cheetah-slideshow-' + this.get('color'));
	...
	_standardResize: function()
	{
        ...
        if(bb.hasClass('ba-cheetah-fullscreen-active')) {
			this._stretchyWindowResize();
			return;
		}
	...
	_playingTimerStart: function(e)
	{
		...
		if(!e && this._playingTimerEvent !== null) {
			this._playingTimerEvent.obj.once('ba-cheetah-slideshow-base|' + this._playingTimerEvent.e, Y.bind(this._playingTimerStart, this));
		}
		else {
			this._playingTimer = Y.later(this.get('speed'), this, this._playingTimerComplete);
		}
	...
	_playingTimerCancel: function()
	{
		...
		if(this._playingTimerEvent) {
			this._playingTimerEvent.obj.detach('ba-cheetah-slideshow-base|' + this._playingTimerEvent.e);
		}
	},
	...
	_renderLoadingImage: function()
	{
		...
		if(this.get('loadingImageEnabled')) {
            ...
            this._loadingImage = new Y.BA.Spinner(settings);

            // Loading image wrap
            this._loadingImageWrap = Y.Node.create('<div class="ba-cheetah-loading-image"></div>');
	 ...
	 * @static
	 */
	CSS_PREFIX: 'ba-cheetah-slideshow-base',
...
}, '2.0.0' ,{requires:['node', 'base', 'widget', 'widget-parent', 'widget-child', 'ba-cheetah-slideshow-album-loader', 'ba-cheetah-spinner']});


YUI.add('ba-cheetah-smugmug-api', function(Y) {

/**
 * @module ba-cheetah-smugmug-api
 */
 ...
 * @extends Base
 */
Y.namespace('BA').SmugMugAPI = Y.Base.create('ba-cheetah-smugmug-api', Y.Base, [], {
