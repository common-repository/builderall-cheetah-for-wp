/**
 * Custom color picker for Cheetah Builder based on Iris by Matt Wiebe.
 *
 ...
 */
var BACheetahColorPicker;

(function( $, undef ) {

	var BACheetahColorPresets 	= [],
	...
	function stupidIEGradient( origin, stops ) {
		...
		$.each(stops, function( i, startColor ) {
			...
			startColor.octoHex = new BACheetahColor( startColor.color ).toIEOctoHex();
			endColor.octoHex = new BACheetahColor( endColor.color ).toIEOctoHex();
	 ...
	 * @method baCheetahParseColorValue
	 * @return {Array}
	 */
	baCheetahParseColorValue = function( val ) {
	 ...
	 * @method $.fn.baCheetahColorPickerGradient
	 * @return {Array}
	 */
	$.fn.baCheetahColorPickerGradient = function() {
		var args = arguments;
	 ...
	 * @method $.fn.baCheetahColorPickerRaninbowGradient
	 * @return {Array}
	 */
	$.fn.baCheetahColorPickerRaninbowGradient = function( origin, args ) {
		...
		return this.each(function() {
			$(this).baCheetahColorPickerGradient( origin, steps );
		});
	};

	/**
	 * Main color picker class for Cheetah Builder's custom implementation.
	 *
	 * @class BACheetahColorPicker
	 * @param {Object} settings

	 */
	BACheetahColorPicker = function( settings )
	{
		this._html  = '<div class="ba-cheetah-color-picker-ui"><div class="iris-picker"><div class="iris-picker-inner"><div class="iris-square"><a class="iris-square-value" href="javascript:void(0);"><span class="iris-square-handle ui-slider-handle"></span></a><div class="iris-square-inner iris-square-horiz"></div><div class="iris-square-inner iris-square-vert"></div></div><div class="iris-slider iris-strip"><div class="iris-slider-offset"></div></div></div></div></div>';
	 ...
	 */
	BACheetahColorPicker.prototype = {
		...
		_init: function(){
			...
			if( $('html').hasClass( 'ba-cheetah-color-picker-init' ) ){
				this._prepareColorFields();
				return;
			}

			this._color = new BACheetahColor( '' ).setHSpace( self.options.mode );

			// Set picker color presets
			BACheetahColorPresets = this.options.presets;
			...
			this._ui 	  = $( '.ba-cheetah-color-picker-ui' );
			this._iris 	  = $( '.iris-picker' );
			this._wrapper = $( 'body' );

			this._ui
				.append( this._hexHtml )
				.append( this._presetsHtml );

			self.element = this._ui.find( '.ba-cheetah-color-picker-input' );
			self._initControls();
			...
			$('html').addClass( 'ba-cheetah-color-picker-init' );
		...
		_prepareColorFields: function(){
			...
			$('.ba-cheetah-color-picker-value').each( function(){

				var $this 			= $( this ),
					$colorValue 	= $this.val(),
					$colorTrigger 	= $this.parent().find( '.ba-cheetah-color-picker-color' ),
					$parsedValue 	= baCheetahParseColorValue( $colorValue ),
					$bgColor 		= '';

				if( $colorValue ){
					// set initial color, check for alpha support
					if ( $colorTrigger.hasClass('ba-cheetah-color-picker-alpha-enabled') && $parsedValue.rgba ) {
						$bgColor = $this.val().toString();
					}
					else if ( !$colorTrigger.hasClass('ba-cheetah-color-picker-alpha-enabled') && $parsedValue.rgba ) {
		...
		_setTemplates: function(){
			...
			this._alphaHtml = '<div class="ba-cheetah-alpha-wrap">' +
								  	'<div class="ba-cheetah-alpha-slider"></div>' +
                                  	'<div class="ba-cheetah-alpha-slider-offset"></div>' +
                                  	'<div class="ba-cheetah-alpha-text"></div>' +
                              '</div>';

			this._presetsHtml = '<div class="ba-cheetah-color-picker-presets">' +
					'<span class="ba-cheetah-color-picker-presets-title">' + this.options.labels.colorPresets +'</span>' +
					'<ul class="ba-cheetah-color-picker-presets-list"></ul>' +
				'</div>';

			this._hexHtml = '<input type="text" class="ba-cheetah-color-picker-input" placeholder="' + this.options.labels.placeholder + '">';

			this._presetsAdd = '<div title="Save current color to your palette" class="ba-cheetah-color-picker-preset-add"></div>'

			this._presetsTpl = '<li class="ba-cheetah-color-picker-preset"><span class="ba-cheetah-color-picker-preset-color"></span> <span class="ba-cheetah-color-picker-preset-label"></span> <span class="ba-cheetah-color-picker-preset-remove ba-cheetah-color-picker-icon-remove"></span></li>';

			this._noPresetsTpl = '<li class="ba-cheetah-color-picker-no-preset"><span class="ba-cheetah-color-picker-preset-label">' + this.options.labels.noPresets + '</span></li>';
		...
		_buildUI: function(){
			var self = this;
			self._presetsList = this._ui.find( '.ba-cheetah-color-picker-presets-list' );
			self._presetsList.html('');

			self._presetsList.append( this._presetsAdd );
		...
		_addPresetView: function( val ){
			...
			var hasEmpty = this._presetsList.find( '.ba-cheetah-color-picker-no-preset' );
			...
			var tpl   = $( this._presetsTpl ),
				color = BACheetahColor( val );

			tpl
				.attr( 'data-color', val )
				.find( '.ba-cheetah-color-picker-preset-color' )
					.css({ backgroundColor: color.toString() })
					.end()
				.find( '.ba-cheetah-color-picker-preset-label' )
					.html( color.toString() );
		...
		_addPresetFeedback: function(){
			...
			this._ui.append( '<div class="ba-cheetah-color-picker-added"><div class="ba-cheetah-color-picker-added-text"><div class="ba-cheetah-color-picker-icon-check"></div> "' + this.options.labels.presetAdded.replace( '%s', this._color.toString() ) + '"</div></div>' );
			this._ui
				.find( '.ba-cheetah-color-picker-added' )
					.hide()
					.fadeIn( 200 )
					.delay( 2000 )
		...
		_pickerControls: function(){
			...
			this._wrapper
				.on( 'click', '.ba-cheetah-color-picker-color', function(){
					var $this = $(this);
					self._currentElement = $this.parent().find('.ba-cheetah-color-picker-value');
				...
				.on( 'click', '.ba-cheetah-color-picker-clear', function(){
					var $this = $(this);
					self._currentElement = $this.parent().find('.ba-cheetah-color-picker-value');

					$this
						.parent().find( '.ba-cheetah-color-picker-color' )
						.css({ backgroundColor: 'transparent' })
						.addClass('ba-cheetah-color-picker-empty');

					$this
						.parent().find('.ba-cheetah-color-picker-value-input')
						.val( '' )
				...
				.on( 'input', '.ba-cheetah-color-picker-value-input', function(){
					var $this = $(this)
					self._currentElement = $this.parent().find('.ba-cheetah-color-picker-value');

					var color = new BACheetahColor( $this.val() ),
						val = $this.val().replace( /^#/, '' )

						if ( color.error || color.toString == '') {
							console.warn('Invalid color')
						} else {
							self._currentElement
								.parent()
								.find( '.ba-cheetah-color-picker-color' )
								.css({ backgroundColor: BACheetahColor( val ).toString() })
								.removeClass( 'ba-cheetah-color-picker-empty' );
			...
			$( document ).on( 'mousedown', function( event ) {
				if ( 0 === $( event.target ).closest( '.ba-cheetah-color-picker-ui' ).length ) {
					presets = self._ui.find( '.ba-cheetah-color-picker-presets' );
					presetsList = presets.find( '.ba-cheetah-color-picker-presets-list' );
					$( '.ba-cheetah-color-picker-ui.ba-cheetah-color-picker-active' ).removeClass( 'ba-cheetah-color-picker-active' );
				}
			});
		...
		_presetsControls: function(){
			var self 		      = this,
				addPreset         = self._ui.find( '.ba-cheetah-color-picker-preset-add' ),
				presets 	      = self._ui.find( '.ba-cheetah-color-picker-presets' ),
				presetsList 	  = presets.find( '.ba-cheetah-color-picker-presets-list' );
			...
			presets
				...
				.on( 'click', '.ba-cheetah-color-picker-preset', function( e ){
					var currentColor = new BACheetahColor( $( this ).data( 'color' ).toString() );
				...
				// removes a preset
				.on( 'click', '.ba-cheetah-color-picker-preset-remove', function( e ){
					e.stopPropagation();
		...
		_removePreset: function( preset ){
			if( confirm( this.options.labels.removePresetConfirm ) ){
				var color = preset.toString(),
					index = BACheetahColorPresets.indexOf( color );

				if( index > -1 ){
					BACheetahColorPresets.splice( index, 1 );
					this.options.presets = BACheetahColorPresets;

					this._presetsList
						.find('.ba-cheetah-color-picker-preset[data-color="'+ color +'"]' )
						.slideUp( function(){
							$( this ).remove();
						});
				}

				if( BACheetahColorPresets.length < 1 ){
					this._presetsList.append( this._noPresetsTpl );
				}

				// CALLBACK FOR PRESET REMOVED
				$(this).trigger( 'presetRemoved', { presets: BACheetahColorPresets } );
		...
		_addPreset: function( preset ){
			...
			} else if( BACheetahColorPresets.indexOf( color ) > -1 ){
			...
			// add color to presets, fires visual feedback and triggers an event
				...
				BACheetahColorPresets.push( color );
				this.options.presets = BACheetahColorPresets;

				// CALLBACK FOR COLOR ADDED
				$(this).trigger( 'presetAdded', { presets: BACheetahColorPresets } );
			}
		},
		...
		_togglePicker: function( position ){
			...
			if( this._ui.hasClass( 'ba-cheetah-color-picker-active' ) ){
				// if the picker is open, hides first, then changes the position
				this._ui.removeClass( 'ba-cheetah-color-picker-active' );

				if( position ){
					setTimeout(	function(){
						self._ui.css( position );
						self._ui.addClass( 'ba-cheetah-color-picker-active' );
		...
		_paintDimension: function( origin, control ) {
			...
			switch ( controlOpts[ control ] ) {
				case 'h':
					...
					I don't want the hue control to change depending on Saturation and Lightness. 
					Instead, keep your colors alive at all times!

					*/

					stops = { s: 100, l: 50 };

					target.baCheetahColorPickerRaninbowGradient( origin, stops );
					break;
				case 's':
					if ( mode === 'hsv' ) {
						if ( control === 'vert' ) {
					...
					target.baCheetahColorPickerGradient( origin, stops );
					break;
				case 'l':
					if ( control === 'strip' ) {
		...
		_addInputListeners: function( input ) {
			var self = this,
				...
				callback = function( event ){
					var color = new BACheetahColor( input.val() ),
						val = input.val().replace( /^#/, '' ),
						isPickerEmpty = self._currentElement.hasClass( 'ba-cheetah-color-picker-empty' );
					...
					if ( color.error ) {
						...
						if ( color.toString() !== self._color.toString() || ( '' !== self._color.toString() && isPickerEmpty ) ) {
							...
							if( event.type === 'keyup' ){
								...
								hex = BACheetahColor( val ).toString();
		...
		_setColor: function( value ) {
			...
			newColor = new BACheetahColor( value ).setHSpace( self.options.mode );
			if ( newColor.error ) {
					...
					if( this._currentElement ){
						...
						if ( ! self.default || (self.default && 'external' !== self.active ) ) {
							this._currentElement
								...
								.removeClass( 'ba-cheetah-color-picker-empty' )
								.parent()
								.find('.ba-cheetah-color-picker-value-input')
								.val(self._color.toString())
						}
						else {
							this._currentElement.val( '' );
						}

						self._wrapper.find('.ba-cheetah-alpha-slider-offset').css('background-color', self._color.toString());
		...
		_buildAlphaUI: function() {
			...
			self._wrapper.on( 'click', '.ba-cheetah-color-picker-color', function(){
				var $this 			= $(this),
					$currentColor   = self._currentElement.val();

				if ( $this.hasClass('ba-cheetah-color-picker-alpha-enabled') ) {

					// Add alpha if not exists
					if (self._ui.find('.ba-cheetah-alpha-wrap').length <= 0) {
						$(self._alphaHtml).insertAfter( self._iris );
					}

					self.picker.addClass('ba-cheetah-color-alpha-enabled');
					self._pickerAlphaControls();
				}
				else {
					self.picker.removeClass('ba-cheetah-color-alpha-enabled');
					self._ui.find('.ba-cheetah-alpha-wrap').remove();
				}
			});
		},
		...
		_pickerAlphaControls: function() {
			var self 		= this,
				...
				picker 		= baCheetahParseColorValue( el.val() ),
				floatValue  = parseFloat( picker.alpha / 100 ),
				wrapper 	= self._wrapper,
                container  	= self._ui,
                alphaWrap   = container.find('.ba-cheetah-alpha-wrap'),
                alphaSlider = alphaWrap.find('.ba-cheetah-alpha-slider'),
                alphaText   = alphaWrap.find('.ba-cheetah-alpha-text'),
                alphaOffset = alphaWrap.find('.ba-cheetah-alpha-slider-offset');
            ...
            alphaSlider.slider({
              	...
              	create: function() {
	                ...
	                // Clear alpha values
	                wrapper.on('click', '.ba-cheetah-color-picker-clear', function() {
	                  	self._color._alpha = 1;
	                  	alphaText.text('');
