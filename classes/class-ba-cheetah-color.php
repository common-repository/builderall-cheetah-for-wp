...
final class BACheetahColor {
	 ...
	 * @param string $fallbackColor String for return if color is empty
	 * @return string
	 */
	static public function hex_or_rgb( $color, $fallbackColor = '' ) {
		if ( empty( $color ) || $color == '') {
			return $fallbackColor;
		}
		else if ( ! empty( $color ) && ! stristr( $color, 'rgb' ) && ! stristr( $color, '#' ) ) {
			$color = '#' . $color;
		}
		return $color;
	}
	...
	static public function is_valid_gradient($gradient = array()) {
		return !empty($gradient['type']) 
			&& !empty($gradient['angle'])
			&& !empty($gradient['position'])
			&& !in_array('', $gradient['colors'])
			&& !in_array('', $gradient['stops']);
	}
	 ...
	 * @return string
	 */
	static public function shadow( $setting, $property = null ) {
		$shadow = '';
		...
		if ( isset( $setting['color'] ) && '' !== $setting['color'] ) {
			...
			if ( isset( $setting['spread'] ) && $property != 'filter') {
	...
	static public function hex_or_rgb_or_gradient($type = 'solid', $solid = '', $gradient = '') {
		if ($type == 'solid' && !empty( $solid )) {
			return self::hex_or_rgb( $solid );
		}
		else if ($type ==  'gradient' && !empty( $gradient )) {
