...
final class BACheetahLoop {
	...
	static public function init() {
		// Actions
		add_action( 'ba_cheetah_before_control_suggest', __CLASS__ . '::render_match_select', 10, 4 );
		add_action( 'init', __CLASS__ . '::init_rewrite_rules', 20 );
		add_action( 'ba_cheetah_activated', __CLASS__ . '::init_rewrite_rules', 10 );
	...
	static public function query( $settings ) {
		 ...
		 * @see ba_cheetah_loop_before_query_settings
		 */
		$settings = apply_filters( 'ba_cheetah_loop_before_query_settings', $settings );

		/**
		 * Before query is performed.
		 * @see ba_cheetah_loop_before_query
		 */
		do_action( 'ba_cheetah_loop_before_query', $settings );
		...
		if ( isset( $settings->order_by ) && 'rand' == $settings->order_by ) {
			if ( isset( $settings->pagination ) && in_array( $settings->pagination, array( 'scroll', 'load_more' ) ) ) {
				if ( ! isset( $_GET['ba_cheetah_rand_seed'] ) ) {
					self::$rand_seed = rand();
				} else {
					self::$rand_seed = absint( $_GET['ba_cheetah_rand_seed'] );
				}
			}
		}
		 ...
		 * @see ba_cheetah_loop_after_query
		 */
		do_action( 'ba_cheetah_loop_after_query', $settings );

		/**
		 * Filter the query results.
		 * @see ba_cheetah_loop_query
		 */
		return apply_filters( 'ba_cheetah_loop_query', $query, $settings );
	}
	...
	static public function main_query() {
		...
		if ( self::$loop_counter > 1 || $wp_the_query->is_singular( 'ba-cheetah-theme-layout' ) ) {

			$query_args = $wp_query->query_vars;

			$query_args['paged']              = self::get_paged();
			$query_args['ba_cheetah_original_offset'] = 0;
			$query_args['ba_cheetah_loop']    = true;
	...
	static public function custom_query( $settings ) {
		...
		$args = array(
			...
			'offset'              => $paged_offset,
			'ba_cheetah_original_offset'  => $offset,
			'ba_cheetah_loop'     => true,
			'fields'              => $fields,
			'settings'            => $settings,
		);
		 ...
		 * @see ba_cheetah_loop_query_args
		 */
		$args = apply_filters( 'ba_cheetah_loop_query_args', $args );
	...
	static public function found_posts( $found_posts, $query ) {
		if ( isset( $query->query ) && isset( $query->query['ba_cheetah_loop'] ) ) {
			return (int) $found_posts - (int) $query->query['ba_cheetah_original_offset'];
		}
	...
	static public function init_rewrite_rules() {
		...
		$flpaged_rules = apply_filters( 'ba_cheetah_loop_rewrite_rules', $flpaged_rules );
	...
	static public function override_canonical( $redirect_url, $requested_url ) {
		...
		if ( is_array( $wp_the_query->query ) ) {
			...
			if ( BACheetahModel::is_builder_enabled() && ! empty( $modules ) ) {
			...
			if ( ba_cheetah_theme_builder_has_post_grid() ) {
				return false;
			}
		}
	...
	static public function pre_404_pagination( $prevent_404, $query ) {
		...
		if ( $query->is_archive && $query->is_category && $query->post_count < 1 ) {
			...
			$post_grid_posts = ba_cheetah_theme_builder_cat_archive_post_grid( $query );
	...
	static public function pagination( $query ) {
		...
		if ( $total_pages > 1 ) {
			...
			if ( self::$rand_seed > 0 ) {
				$add_args['ba_cheetah_rand_seed'] = self::$rand_seed;
			}

			/**

			 * @see ba_cheetah_loop_paginate_links_args
			 */
			$args = apply_filters( 'ba_cheetah_loop_paginate_links_args', array(
				'base'     => $base . '%_%',
				'format'   => $format,
	...
	static public function post_types() {
		...
		unset( $post_types['ba-cheetah-theme-layout'] );
	...
	static public function taxonomies( $post_type ) {
		...
		/**
		 * @see ba_cheetah_loop_taxonomies
		 */
		return apply_filters( 'ba_cheetah_loop_taxonomies', $data, $taxonomies, $post_type );
	}
	...
	static public function render_match_select( $name, $value, $field, $settings ) {
		...
		$label = BACheetahUtils::strtolower( $field['label'] );

		if ( ! isset( $settings->{ $name . '_matching' } ) ) {
			$settings->{ $name . '_matching' } = '1';
		}

		include BA_CHEETAH_DIR . 'includes/loop-settings-matching.php';
	}
}
