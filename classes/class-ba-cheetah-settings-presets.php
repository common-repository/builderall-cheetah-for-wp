<?php
class BACheetahSettingsPresets {

	static private $presets = array();

	static public function init() {

		add_filter( 'ba_cheetah_shared_option_sets', 'BACheetahSettingsPresets::filter_shared_option_sets' );
		add_filter( 'ba_cheetah_register_settings_form', 'BACheetahSettingsPresets::filter_settings_form' );

		/**
		 * Register presets action.
		 * @see ba_cheetah_register_presets
		 */
		do_action( 'ba_cheetah_register_presets' );
	}
	...
	*/
	static public function register( $type = '', $args = array() ) {
		$defaults                        = array(
			'name'     => '',
			'label'    => __( 'Untitled Preset', 'ba-cheetah' ),
			'type'     => $type,
