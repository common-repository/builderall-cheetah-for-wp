 ...
 */
class BACheetahSettingsCompatHelper {
	...
	public function filter_child_module_settings( $slug, &$settings, $key_map ) {
		...
		if ( isset( BACheetahModel::$modules[ $slug ] ) ) {
			$module = BACheetahModel::$modules[ $slug ];
		} else {
			return;
		}
	...
	public function handle_opacity_inputs( &$settings, $opacity_key, $color_key ) {
		...
		if ( (int) $settings->$opacity_key < 100 && ! stristr( $settings->$color_key, 'rgb' ) ) {
			$settings->$color_key = 'rgba(' . implode( ',', BACheetahColor::hex_to_rgb( $settings->$color_key ) ) . ',' . $settings->$opacity_key / 100 . ')';
		}
