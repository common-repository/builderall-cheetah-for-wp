...
final class BACheetahModel {
	 ...
	 * or from the ba_cheetah_data post variable.
	 *
	 ...
	 * $post->ID when calling the BACheetahModel::get_post_id method.
	 *
	 * The first post ID in the array is always returned for the
	 * BACheetahModel::get_post_id method. To get a previously set
	 * post ID, first call the BACheetahModel::reset_post_id method.
	 *
	...
	/**
	 * An array of real elements name.
	 *

	 * @access private
	 * @var array $elementsKey
	 */
	static private $elementsKey = array(
		'row' => 'rows',
		'column-group' => 'groups',
		'column' => 'columns',
		'module' => 'modules',
	...
	static public function init() {
		/* Admin AJAX */
		add_action( 'wp_ajax_ba_cheetah_disable', __CLASS__ . '::disable' );
		add_action( 'wp_ajax_ba_cheetah_duplicate_wpml_layout', __CLASS__ . '::duplicate_wpml_layout' );
		...
		add_action( 'save_post', __CLASS__ . '::save_mode', 10, 3 );
		add_action( 'wp_restore_post_revision', __CLASS__ . '::restore_revision', 10, 2 );
		add_action( 'ba_cheetah_after_save_layout', __CLASS__ . '::save_layout_revision' );
		add_action( 'ba_cheetah_after_save_user_template', __CLASS__ . '::save_layout_revision' );

		/* Filters */
		add_filter( 'heartbeat_received', __CLASS__ . '::lock_post', 10, 2 );
		add_filter( 'ba_cheetah_register_settings_form', __CLASS__ . '::filter_row_settings_for_resize', 10, 2 );
	...
	static public function get_edit_url( $post_id = false ) {
		...
		//$url = set_url_scheme( add_query_arg( 'ba_cheetah', '', get_permalink( $post->ID ) ), $scheme );
		$url = set_url_scheme( add_query_arg( 'ba_builder', '', get_permalink( $post->ID ) ), $scheme );

		/**
		 * Filter the bb edit url.
		 * @see ba_cheetah_get_edit_url
		 * @param $url url
		 * @param $post post object
		 */
		return apply_filters( 'ba_cheetah_get_edit_url', $url, $post );
	}

	/**
	 * Returns the URL to upgrade the builder to the pro version.
	 * Can be overridden by theme developers to use their affiliate
	 * link using the ba_cheetah_upgrade_url filter.
	 *
	...
	static public function get_upgrade_url( $params = array() ) {
		/**
		 * Use this filter to modify the upgrade URL in Cheetah Builder Lite.
		 * This can be used to add an affiliate ID.
		 * @see ba_cheetah_upgrade_url
		 */
		return apply_filters( 'ba_cheetah_upgrade_url', self::get_link_url( '', $params ) );
	}

	/**
	 * Returns a URL that points to the Cheetah Builder store.
	 *
	 ...
	 * @return string
	 */
	static public function get_landingpage_url( $path = '', $params = array() ) {
		return BA_CHEETAH_LANDINGPAGE_URL;
	}

	/**
	 * Returns a URL that points to the Cheetah Builder Oauth.
	 *
	...
	static public function get_relative_plugin_url() {
		$url         = str_ireplace( home_url(), '', BA_CHEETAH_URL );
		$parsed_path = parse_url( BA_CHEETAH_URL, PHP_URL_PATH );
	...
	/**
	 * Returns an array of post data from either $_POST['ba_cheetah_data']
	 * or $_POST if that is not set.
	 *

	 * @return array
	 */
	static public function get_cheetah_ba_data() {
		if ( ! self::$post_data ) {

			self::$post_data = array();

			if ( isset( $_POST['ba_cheetah_data'] ) ) {
				// Decode settings if our ModSecurity fix is enabled.
				if ( isset( $_POST['ba_cheetah_data']['settings'] ) ) {
					$_POST['ba_cheetah_data']['settings'] = BACheetahUtils::modsec_fix_decode( $_POST['ba_cheetah_data']['settings'] );
				}
				if ( isset( $_POST['ba_cheetah_data']['node_settings'] ) ) {
					$_POST['ba_cheetah_data']['node_settings'] = BACheetahUtils::modsec_fix_decode( $_POST['ba_cheetah_data']['node_settings'] );
				}

				if ( isset( $_POST['ba_cheetah_data']['node_preview'] ) ) {
					$_POST['ba_cheetah_data']['node_preview'] = BACheetahUtils::modsec_fix_decode( $_POST['ba_cheetah_data']['node_preview'] );
				}

				$data = BACheetahUtils::json_decode_deep( wp_unslash( $_POST['ba_cheetah_data'] ) );
	...
	static public function update_post_data( $key, $value ) {
		$post_data         = self::get_cheetah_ba_data();
		$post_data[ $key ] = $value;
	...
	static public function get_post_types() {
		$value = self::get_admin_settings_option( '_ba_cheetah_post_types', true );
		 ...
		 * @see ba_cheetah_post_types
		 */
		return apply_filters( 'ba_cheetah_post_types', $value );
	}
	...
	static public function get_global_posts() {
		 ...
		 * @see ba_cheetah_global_posts

		 */
		return apply_filters( 'ba_cheetah_global_posts', array() );
	}
	 ...
	 * $post->ID will be used when calling BACheetahModel::get_post_id.
	 *
	...
	static public function is_post_editable() {
		 ...
		 */
		return (bool) apply_filters( 'ba_cheetah_is_post_editable', $editable );
	}
	...
	static public function lock_post( $response, $data ) {
		if ( isset( $data['ba_cheetah_post_lock'] ) ) {

			require_once ABSPATH . 'wp-admin/includes/post.php';

			wp_set_post_lock( $data['ba_cheetah_post_lock']['post_id'] );
		}
		return $response;
	}
	...
	static public function is_builder_enabled( $post_id = null ) {
		...
		if ( ! is_admin() && post_password_required( $post_id ) ) {
			...
			if ( $post && in_array( $post->post_type, $post_types ) ) {
				return get_post_meta( $post->ID, '_ba_cheetah_enabled', true );
			}
		}
	...
	static public function is_builder_active() {
		...
		if ( null !== self::$active ) {
			...
			$post_data    = self::get_cheetah_ba_data();
			self::$active = isset( $_GET['ba_cheetah'] ) || isset( $post_data['ba_cheetah']) || isset( $_GET['ba_builder'] ) || isset( $post_data['ba_builder'] );
		}

		return self::$active;
	}

	static public function is_builder_in_debug_tree_mode()
	{
		return self::is_builder_active() && isset($_GET['treedebug']);
	}
	...
	static public function is_builder_draft_preview() {
		return is_user_logged_in() && isset( $_GET['ba_cheetah_preview'] );
	}
	...
	static public function is_new_user() {
		if ( self::is_builder_active() ) {
			...
			$launched     = get_user_meta( $current_user->ID, '_ba_cheetah_launched_tour', true );

			if ( empty( $launched ) ) {
				update_user_meta( $current_user->ID, '_ba_cheetah_launched_tour', 1 );
				return true;
			}
		}
	...
	static public function get_node_status() {
		...
		/**
		 * @see ba_cheetah_node_status
		 */
		return apply_filters( 'ba_cheetah_node_status', $status );
	}
	...
	static public function enable() {
		update_post_meta( self::get_post_id(), '_ba_cheetah_enabled', true );
	}
	...
	static public function disable() {
		update_post_meta( self::get_post_id(), '_ba_cheetah_enabled', false );
	}
	...
	static public function enable_editing() {
		...
		if ( self::is_post_editable() && is_object( $wp_the_query->post ) ) {
			 ...
			 * @see ba_cheetah_migrated_post_content
			 */
			$content = apply_filters( 'ba_cheetah_migrated_post_content', $post->post_content );
			...
			if ( empty( $published ) && empty( $draft ) && ! empty( $content ) ) {
				...
				// These lines reset the page's css and js every time the editor was reloaded and the post was in draft status
			 ...
			 * @see ba_cheetah_editing_enabled
			 */
			do_action( 'ba_cheetah_editing_enabled' );
		}
	}
	...
	static public function get_upload_dir() {
		...
		$dir_name = basename( BA_CHEETAH_DIR );
		...
		if ( ! ba_cheetah_filesystem()->file_exists( $dir_info['path'] ) ) {

			// Create the directory.
			ba_cheetah_filesystem()->mkdir( $dir_info['path'] );

			// Add an index file for security.
			ba_cheetah_filesystem()->file_put_contents( $dir_info['path'] . 'index.html', '' );
		}
		 ...
		 * @see ba_cheetah_get_upload_dir
		 */
		return apply_filters( 'ba_cheetah_get_upload_dir', $dir_info );
	}
	...
	static public function get_cache_dir( $name = 'cache' ) {
		 ...
		 * @see ba_cheetah_get_cache_dir
		 */
		return apply_filters( 'ba_cheetah_get_cache_dir', $dir_info );
	}
	...
	static public function get_asset_info() {
		$post_data = array_merge(self::get_cheetah_ba_data(), $_POST);
		$post_id   = self::get_post_id();
		...
		/**
		 *
		 * @see ba_cheetah_get_asset_info_post_id

		 */
		$post_id = apply_filters( 'ba_cheetah_get_asset_info_post_id', $post_id, $post_data, $active, $preview );
	 ...
	 * method is used. Return true for the ba_cheetah_render_assets_inline
	...
	static public function get_asset_enqueue_method() {
		 ...
		 * @see ba_cheetah_render_assets_inline
		 */
		return apply_filters( 'ba_cheetah_render_assets_inline', false ) ? 'inline' : 'file';
	}
	...
	static public function delete_asset_cache( $type = false ) {
		...
		foreach ( $types as $type ) {
			...
			if ( isset( $info[ $type ] ) && ba_cheetah_filesystem()->file_exists( $info[ $type ] ) ) {
				ba_cheetah_filesystem()->unlink( $info[ $type ] );
			}
		}
	}

	static public function set_cheetah_mode ()
	{
		$post_id = self::get_post_id();

		if($post_id && isset($_POST['ba_cheetah_data'], $_POST['ba_cheetah_data']['canvas_enabled'])) {
			$enabled = $_POST['ba_cheetah_data']['canvas_enabled'] === 'true';
			update_post_meta($post_id, 'ba_cheetah_mode', $enabled ? 'canvas' : 'default');
			update_post_meta( $post_id, '_wp_page_template', $enabled ? 'ba-cheetah-canvas-template' : 'default');
		}
	...
	static public function delete_all_asset_cache( $post_id = false ) {
		...
		if ( $post_id && 'file' === BACheetahModel::get_asset_enqueue_method() ) {
			...
			foreach ( $paths as $path ) {
				if ( ba_cheetah_filesystem()->file_exists( $path ) ) {
					ba_cheetah_filesystem()->unlink( $path );
				}
			}
		}
	}
	...
	static public function delete_asset_cache_for_all_posts( $parts = '*' ) {
		...
		if ( is_array( $css ) ) {
			array_map( array( ba_cheetah_filesystem(), 'unlink' ), $css );
		}
		if ( is_array( $js ) ) {
			array_map( array( ba_cheetah_filesystem(), 'unlink' ), $js );
		}
	}
	...
	static public function get_nodes( $type = null, $parent_id = null, $status = null ) {
		...
		uasort( $nodes, array( 'BACheetahModel', 'order_nodes' ) );
	...
	static public function get_child_nodes( $parent_id, $status = null ) {
		...
		if ( $template_post_id ) {
			$template_node_id = apply_filters( 'ba_cheetah_parent_template_node_id', $parent->template_node_id, $parent, $data );
		}
	 ...
	 * @param array $nodes References pass.
	 * @param string $parent_id The parent node id.
	 * @param string $type The current type. Either "column", "column-group" or "row".
	 * @return array
	 */
	static public function get_nodes_below(&$nodes, $parents = null, $type = 'row') {
		$elements_flip = array_flip(self::$elements);

		if($parents == null) {
			$parents = self::get_nodes('row');
		}
		...
		foreach ( $parents as $parent ) {
			...
			if ( 'module' == $parent->type ) {
				$node = self::get_module( $parent );
			}
			$nodes[self::$elementsKey[$type]][ $parent->node ] = $node;

			for($i = 0; $i < count(self::$elements); $i++) {

				$nextNode = self::get_nodes( self::$elements[$i], $parent );
				if(!empty($nextNode)) {
					$strElement = ('row-inside' === self::$elements[$i]) ? 'row' : self::$elements[$i];
					self::get_nodes_below($nodes, $nextNode, $strElement);
				}
			}
		}
		return;
	...
	static public function get_categorized_nodes() {
		...
		if ( self::is_post_user_template( 'module' ) ) {
			...
			self::get_nodes_below($nodes, $rows, 'row');
		}
	...
	static public function get_node_settings( $node, $filter = true ) {
		...
		return ! $filter ? $settings : apply_filters( 'ba_cheetah_node_settings', $settings, $node );
	}
	...
	static public function get_node_settings_with_defaults_merged( $type, $settings ) {
		...
		if ( 'row' == $type ) {
			$defaults = BACheetahModel::get_row_defaults();
		} elseif ( 'column' == $type ) {
			$defaults = BACheetahModel::get_col_defaults();
		} elseif ( 'module' == $type ) {
			$defaults = BACheetahModel::get_module_defaults( $settings->type );
		}

		$settings = (object) array_merge( (array) $defaults, (array) $settings );

		if ( 'row' == $type ) {
			$settings = BACheetahModel::merge_nested_form_defaults( 'general', 'row', $settings );
		} elseif ( 'column' == $type ) {
			$settings = BACheetahModel::merge_nested_form_defaults( 'general', 'col', $settings );
		} elseif ( 'module' == $type ) {
			$settings = BACheetahModel::merge_nested_module_defaults( $settings->type, $settings );
		}
	 ...
	 * @return object
	 */
	static public function process_node_settings( $node, $new_settings, $saving = false ) {
		...
		if ( 'module' == $node->type ) {
			$new_settings = self::process_module_settings( $node, $new_settings, $saving );
	...
	/**
	 * Adds a element inside another element to the current layout.
	 *
	 ...
	 */
	static public function add_row_to_module($nodeID = null, $parent = null, $module_slug = '', $index = 0 ) {
		$data        = self::get_layout_data();
		$settings    = self::get_row_defaults();
		$row_node_id = (!empty($nodeID)) ? $nodeID : self::generate_node_id();
		$cols		 = '1-col';

		// Add the row.
		$data[ $row_node_id ]           = new StdClass();
		$data[ $row_node_id ]->node     = $row_node_id;
		$data[ $row_node_id ]->type     = 'row-inside';
		$data[ $row_node_id ]->parent   = $parent;
		//$data[ $row_node_id ]->position = self::next_node_position( 'row' );
		$data[ $row_node_id ]->position = 0;
		$data[ $row_node_id ]->settings = $settings;

		// Update the layout data.
		self::update_layout_data( $data );

		BACheetahModel::apply_row_inside_template($row_node_id, $module_slug, $index );
	...
	static public function process_row_settings( $row, $new_settings ) {
		...
		if ( 'video' == $new_settings->bg_type ) {
			...
			if ( 'wordpress' == $new_settings->bg_video_source ) {
				// Video MP4
				$mp4 = BACheetahPhoto::get_attachment_data( $new_settings->bg_video );
				...
				// Video WebM
				$webm = BACheetahPhoto::get_attachment_data( $new_settings->bg_video_webm );
	...
	static public function get_row_slideshow_source( $row ) {
		...
		if ( ! class_exists( 'BACheetahSlideshowModule' ) ) {
			return false;
		}

		// Hijack the slideshow module to get the source.
		$ss							= new BaCheetahSlideshowModule();
	...
	static public function add_col_parent( $parent_id = null, $position = null ) {
		...
		} elseif ( 'row' == $parent->type || 'row-inside' == $parent->type ) {
	...
	static public function load_modules() {
		$paths = glob( BA_CHEETAH_DIR . 'modules/*' );
		/**
		 * Filter the modules paths.
		 * @see ba_cheetah_load_modules_paths
		 */
		$paths       = apply_filters( 'ba_cheetah_load_modules_paths', $paths );
		$module_path = '';
		...
		foreach ( $paths as $path ) {
			...
			$child_path   = get_stylesheet_directory() . '/ba-cheetah/modules/' . $module_path;
			$theme_path   = get_template_directory() . '/ba-cheetah/modules/' . $module_path;
			$builder_path = BA_CHEETAH_DIR . 'modules/' . $module_path;
		 ...
		 * @see ba_cheetah_register_extensions
		 */
		do_action( 'ba_cheetah_register_extensions' );
	}
	...
	static public function register_module( $class, $form ) {
		if ( class_exists( $class ) ) {
			 ...
			 * @see ba_cheetah_register_module
			 */
			$instance->enabled = apply_filters( 'ba_cheetah_register_module', $instance->enabled, $instance );

			// Save the instance in the modules array.
			self::$modules[ $instance->slug ]                   = $instance;
			self::$modules[ $instance->slug ]->form             = apply_filters( 'ba_cheetah_register_settings_form', $form, $instance->slug );
			 ...
			 * @see ba_cheetah_register_module_settings_form
			 */
			self::$modules[ $instance->slug ]->form = apply_filters( 'ba_cheetah_register_module_settings_form', self::$modules[ $instance->slug ]->form, $instance->slug );
		}
	}
	...
	static public function register_module_alias( $alias, $config ) {
		...
		$instance->icon        = isset( $config['icon'] ) ? $module->get_icon( $config['icon'] ) : BACheetahModule::get_default_icon();
	...
	static public function get_enabled_modules() {
		$setting = false; // - force enable all modules self::get_admin_settings_option( '_ba_cheetah_enabled_modules', true );
		 ...
		 * @see ba_cheetah_enabled_modules
		 */
		return apply_filters( 'ba_cheetah_enabled_modules', $setting );
	}
	...
	static public function get_module_groups() {
		$groups    = array();
		$templates = BACheetahModel::get_module_templates_data();
		...
		/**
		 * Returns an array of module group slugs and names.
		 * @see ba_cheetah_module_groups

		 */
		return apply_filters( 'ba_cheetah_module_groups', $groups );
	}
	...
	static public function get_module_categories() {
		 ...
		 * @see ba_cheetah_module_categories
		 */
		foreach ( apply_filters( 'ba_cheetah_module_categories', array() ) as $custom_category ) {
			$categories[ $custom_category ] = array();
		}

		// Build the default category arrays.
		$categories[ __( 'Basic', 'ba-cheetah' ) ]    = array();
		$categories[ __( 'Media', 'ba-cheetah' ) ]    = array();
		$categories[ __( 'Layout', 'ba-cheetah' ) ]   = array();
		$categories[ __( 'Animated', 'ba-cheetah' ) ] = array();
		$categories[ __( 'Social', 'ba-cheetah' ) ] = array();
		$categories[ __( 'Builderall Apps', 'ba-cheetah' ) ] = array();
		$categories[ __( 'Other', 'ba-cheetah' ) ]    = array();
	...
	static public function get_categorized_modules( $show_disabled = false ) {
		...
		$other_key       = __( 'Other', 'ba-cheetah' );
		$widgets_key     = __( 'WordPress Widgets', 'ba-cheetah' );
	...
	static public function get_modules_supports_row_inside() {
		return ['accordion', 'tab', 'mega-menu'];
	}
	...
	static public function get_uncategorized_modules( $show_disabled = false ) {
		...
		$modules_order = [
			'heading', 'rich-text', 'button', 'line', 'list', 'spacing', 'html', 'iframe', 	// basics
			'photo', 'video', 'icon', 'gallery', 'slideshow', 'carousel', 'map', 'audio', // media
			'accordion', 'tab', 'menu', 'mega-menu', 'posts', 'testimonials', 'card', 'pricing-table', 'pricing-list', 'flip-box', // layout
			'animated-headline', 'animated-text', 'timer', 'counter', 'progress-bar', 'progress-circle', // animated elements
            'social-icons', 'float-button-icons', 'fb-button', 'fb-comments', 'fb-page', 'fb-save', 'fb-share', 'fb-video' // fb-apps social
		];
		...
		$widgets         = BACheetahModel::get_wp_widgets();
		...
		foreach ( self::$modules as $module ) {
			...
			$module->order 	  = array_search($module->slug, $modules_order);

			if ( ! isset( $module->icon ) || '' == $module->icon ) {
				$module->icon = BACheetahModule::get_default_icon();
			}
		...
		foreach ( $aliases as $alias => $config ) {
			...
			$config->order 	  = array_search($config->slug, $modules_order);
		...
		if ( in_array( 'widget', $enabled_modules ) ) {
			foreach ( $widgets as $widget ) {
				...
				$data->group = array( sanitize_key( __( 'WordPress Widgets', 'ba-cheetah' ) ) );

				if ( ! isset( $widget->icon ) ) {
					$data->icon = BACheetahModule::get_widget_icon();
				}
				$modules[] = $data;
			}
		}
	...
	static public function get_module( $node_id ) {
		...
		if ( self::is_module_registered( $module->settings->type ) ) {
			...
			$instance->dependencies = self::get_module_dependencies($instance->slug);
			$instance->type     	= 'module';
			$instance->form     	= self::$modules[ $module->settings->type ]->form;
			$instance->icon     	= isset( $module->icon ) ? $module->icon : BACheetahModule::get_default_icon();
	...
	static public function get_module_dependencies($module) {
		$dependencies = BACheetah::module_dependencies();
	...
	static public function get_modules( $col_id = null ) {
		...
		foreach ( $modules as $module ) {
			...
			if ( self::is_module_registered( $module->settings->type ) ) {
				...
				$instances[ $i ]->icon     = isset( $module->icon ) ? $module->icon : BACheetahModule::get_default_icon();
	...
	static public function add_default_module( $parent_id = null, $type = null, $position = null, $defaults = null ) {
		...
		if ( ! $parent || 'row' == $parent->type || 'row-inside' == $parent->type || 'column-group' == $parent->type ) {
	 ...
	 * @param boolean $saving call save() module method only when its saved (no in ajax preview)
	 * @return object
	 */
	static public function process_module_settings( $module, $new_settings, $saving = false ) {
		...
		if ($saving) {
			$new_settings	= $instance->save( $new_settings );
		}
	...
	static public function get_wp_widgets() {
		 ...
		 * @see ba_cheetah_get_wp_widgets_exclude
		 */
		$exclude = apply_filters( 'ba_cheetah_get_wp_widgets_exclude', array(
		...
		foreach ( $wp_widget_factory->widgets as $class => $widget ) {
			...
			$widget->category         = __( 'WordPress Widgets', 'ba-cheetah' );
	...
	static public function get_column_groups() {
		$cols = array(
			array(
				'name'  => __( '1 Column', 'ba-cheetah' ),
				'id'    => '1-col',
				'count' => 1,
			),
			array(
				'name'  => __( '2 Columns', 'ba-cheetah' ),
				'id'    => '2-cols',
				'count' => 2,
			),
			array(
				'name'  => __( '3 Columns', 'ba-cheetah' ),
				'id'    => '3-cols',
				'count' => 3,
			),
			array(
				'name'  => __( '4 Columns', 'ba-cheetah' ),
				'id'    => '4-cols',
				'count' => 4,
			),
			array(
				'name'  => __( '5 Columns', 'ba-cheetah' ),
				'id'    => '5-cols',
				'count' => 5,
			),
			array(
				'name'  => __( '6 Columns', 'ba-cheetah' ),
				'id'    => '6-cols',
				'count' => 6,
			),
			array(
				'name'  => __( 'Left', 'ba-cheetah' ),
				'id'    => 'left-sidebar',
				'count' => 2,
			),
			array(
				'name'  => __( 'Right', 'ba-cheetah' ),
				'id'    => 'right-sidebar',
				'count' => 2,
			),
			array(
				'name'  => __( 'Left & Right', 'ba-cheetah' ),
	...
	static public function load_settings() {
		require_once BA_CHEETAH_DIR . 'includes/global-settings.php';
		require_once BA_CHEETAH_DIR . 'includes/layout-settings.php';
		require_once BA_CHEETAH_DIR . 'includes/row-settings.php';
		require_once BA_CHEETAH_DIR . 'includes/column-settings.php';
		require_once BA_CHEETAH_DIR . 'includes/module-settings.php';
		require_once BA_CHEETAH_DIR . 'includes/page-settings.php';
	}
	...
	static public function register_settings_form( $id, $form ) {
		/**
		 * Use this filter to modify the config array for a settings form when it is registered.
		 * @see ba_cheetah_register_settings_form
		 */
		self::$settings_forms[ $id ] = apply_filters( 'ba_cheetah_register_settings_form', $form, $id );
	...
	static public function get_settings_form_fields( $form, $group = null ) {
		...
		if ( 'string' === gettype( $form ) ) {
			if ( 'general' === $group ) {
				$form = BACheetahModel::$settings_forms[ $form ]['tabs'];
			} elseif ( 'module' === $group ) {
				$form = BACheetahModel::$modules[ $form ]->form;
			} else {
				return $fields;
			}
		}
	...
	static public function get_settings_form_defaults( $type ) {
		 ...
		 * @see ba_cheetah_settings_form_defaults
		 */
		self::$settings_form_defaults[ $type ] = apply_filters( 'ba_cheetah_settings_form_defaults', $defaults, $form_type );
	...
	static public function save_settings( $node_id = null, $settings = null ) {
		...
		$new_settings = self::process_node_settings( $node, $new_settings, true );
	...
	static public function sanitize_settings( $settings, $form, $group ) {
		$fields = BACheetahModel::get_settings_form_fields( $form, $group );
	...
	static public function get_global_settings() {
		if ( null === self::$global_settings ) {
			$settings = get_option( '_ba_cheetah_settings' );
	...
	static public function save_global_settings( $settings = array() ) {
		...
		update_option( '_ba_cheetah_settings', $new_settings );

		return self::get_global_settings();
	}

	static public function save_page_settings($settings = array(), $status = null, $post_id = null)
	{

		// Update layout settings
		self::update_layout_settings( $settings, $status, $post_id );

		$post_id = get_the_ID();

		update_post_meta($post_id, '_wp_page_template', $settings['layout']);

		if (isset($settings['ba-cheetah-footer-option'])) {

			if ($settings['ba-cheetah-footer-option'] == 'custom') {
				update_post_meta(
					$post_id,
					'ba-cheetah-custom-footer-id',
					$settings['ba-cheetah-custom-footer-id']
				);
			} else {
			...
			update_post_meta(
				...
				$settings['ba-cheetah-footer-option']
			);
		}

		if (isset($settings['ba-cheetah-header-option'])) {

			if ($settings['ba-cheetah-header-option'] == 'custom') {
				update_post_meta(
					$post_id,
					'ba-cheetah-custom-header-id',
					$settings['ba-cheetah-custom-header-id']
				);
			} else {
			...
			update_post_meta(
				...
				$settings['ba-cheetah-header-option']
			);
		}


		return self::get_page_settings();
	}
	...
	static public function duplicate_post() {
		...
		$data = array(
			...
			'post_title'     => sprintf( _x( 'Copy of %s', '%s stands for post/page title.', 'ba-cheetah' ), $post->post_title ),
		...
		if ( count( $post_meta ) !== 0 ) {
			...
			foreach ( $post_meta as $meta_info ) {
				...
				if ( '_ba_cheetah_template_id' == $meta_key ) {
		...
		$template_id = get_post_meta( $new_post_id, '_ba_cheetah_template_id', true );
		$global      = get_post_meta( $post_id, '_ba_cheetah_template_global', true );
	...
	/**
	 * Limit the amount of revisions possible for ba-cheetah-template type.
	...
	static public function limit_revisions( $num, $post ) {
		...
		if ( 'ba-cheetah-template' == $post->post_type ) {
			/**
			 * Limit the amount of revisions for the ba-cheetah-template type.
			 * @see ba_cheetah_template_revisions
			 */
			$num = apply_filters( 'ba_cheetah_template_revisions', 25 );
		}
		return $num;
	}
	...
	static public function save_layout_revision_changed_filter( $post_has_changed, $last_revision, $post ) {
		...
		$old = serialize( get_post_meta( $last_revision->ID, '_ba_cheetah_data', true ) );
		$new = serialize( get_post_meta( $post->ID, '_ba_cheetah_data', true ) );
	...
	/**
	 * Get all of the layout data for a single file
	 *

	 * @param string $filename Complete path of the file
	 * @return array
	 */

	static public function get_layout_data_from_file($filename) 
	{
		try {
			$template_data = file_get_contents($filename);
			$template_data = unserialize($template_data);
			$template_data = self::clean_layout_data( $template_data );
			$template_data = BACheetahSettingsCompat::filter_layout_data( $template_data );
	...
	static public function get_layout_data( $status = null, $post_id = null ) {
		...
		if ( 'published' == $status || 'revision' == get_post_type( $post_id ) ) {
			if ( isset( self::$published_layout_data[ $post_id ] ) ) {
				...
				$data                                    = get_metadata( 'post', $post_id, '_ba_cheetah_data', true );
				$data                                    = self::clean_layout_data( $data );
				$data                                    = BACheetahSettingsCompat::filter_layout_data( $data );
				self::$published_layout_data[ $post_id ] = apply_filters( 'ba_cheetah_get_layout_metadata', $data, $status, $post_id );
			}
		} elseif ( 'draft' == $status ) {
			if ( isset( self::$draft_layout_data[ $post_id ] ) ) {
				$data = self::$draft_layout_data[ $post_id ];
			} else {
				$data                                = get_metadata( 'post', $post_id, '_ba_cheetah_draft', true );
				$data                                = self::clean_layout_data( $data );
				$data                                = BACheetahSettingsCompat::filter_layout_data( $data );
				self::$draft_layout_data[ $post_id ] = apply_filters( 'ba_cheetah_get_layout_metadata', $data, $status, $post_id );
			}
		}
		...
		/**
		 * Return the data.
		 * @see ba_cheetah_layout_data
		 */
		return apply_filters( 'ba_cheetah_layout_data', $data, $status, $post_id );
	}
	...
	static public function update_layout_data( $data, $status = null, $post_id = null ) {
		$post_id  = ! $post_id ? self::get_post_id() : $post_id;
		$status   = ! $status ? self::get_node_status() : $status;
		$key      = 'published' == $status ? '_ba_cheetah_data' : '_ba_cheetah_draft';
	 ...
	 *
	 * Also makes sure we're not serializing any BACheetahModule
	...
	*/
	...
	static public function get_layout_settings( $status = null, $post_id = null ) {
		$status   = ! $status ? self::get_node_status() : $status;
		$post_id  = ! $post_id ? self::get_post_id() : $post_id;
		$key      = 'published' == $status ? '_ba_cheetah_data_settings' : '_ba_cheetah_draft_settings';
		...
		$settings = (object) array_merge( (array) $defaults, (array) $settings );
		return apply_filters( 'ba_cheetah_layout_settings', $settings, $status, $post_id );
	}

	static public function get_page_settings($status = null, $post_id = null)
	{
		// Layout settings
		$status   = ! $status ? self::get_node_status() : $status;
		$post_id  = ! $post_id ? self::get_post_id() : $post_id;
		...
		$layout_settings = array_merge( (array) $defaults, (array) $settings );

		// Page settings
		$meta = get_post_meta(get_the_ID());
		$template = ''; //get_template_directory_uri();

		if (!empty($meta['_wp_page_template'][0]) && $meta['_wp_page_template'][0] != $template && $meta['_wp_page_template'][0] != 'default') {
			$template = $meta['_wp_page_template'][0];
		}

		$header_option = get_post_meta(get_the_ID(), 'ba-cheetah-header-option', true);
		$footer_option = get_post_meta(get_the_ID(), 'ba-cheetah-footer-option', true);

		$page_settings = [
			'layout' => $template,
			'ba-cheetah-header-option' => $header_option ? $header_option : 'global',
			'ba-cheetah-footer-option' => $footer_option ? $footer_option : 'global',
			'ba-cheetah-custom-header-id' => get_post_meta(get_the_ID(), 'ba-cheetah-custom-header-id', true) ? get_post_meta(get_the_ID(), 'ba-cheetah-custom-header-id', true) :  '0',
			'ba-cheetah-custom-footer-id' => get_post_meta(get_the_ID(), 'ba-cheetah-custom-footer-id', true) ? get_post_meta(get_the_ID(), 'ba-cheetah-custom-footer-id', true) :  '0',
		];

		$settings = (object) array_merge($layout_settings, $page_settings);

		return apply_filters('ba_cheetah_page_settings', $settings, $status, $post_id);
	}
	...
	static public function save_layout( $publish = true ) {
		$editor_content = BACheetah::render_editor_content();
		$post_id        = self::get_post_id();
		 ...
		 * @see ba_cheetah_before_save_layout
		 */
		do_action( 'ba_cheetah_before_save_layout', $post_id, $publish, $data, $settings );
		...
		BACheetah::render_assets();

		/**
		 * This action allows you to hook into after the data is saved for a layout.
		 * @see ba_cheetah_after_save_layout
		 */
		do_action( 'ba_cheetah_after_save_layout', $post_id, $publish, $data, $settings );
	}
	...
	static public function save_draft() {
		 ...
		 * @see ba_cheetah_after_save_draft
		 */
		do_action( 'ba_cheetah_after_save_draft', $post_id, $post_status );
	}
	...
	static public function duplicate_wpml_layout( $original_post_id = null, $new_post_id = null ) {
		$post_data        = array_merge(self::get_cheetah_ba_data(), $_POST);
		$original_post_id = isset( $post_data['original_post_id'] ) ? absint($post_data['original_post_id']) : $original_post_id;
		$new_post_id      = isset( $post_data['post_id'] ) ? absint($post_data['post_id']) : $new_post_id;
		$enabled          = get_post_meta( $original_post_id, '_ba_cheetah_enabled', true );
		...
		if ( ! empty( $enabled ) ) {
			update_post_meta( $new_post_id, '_ba_cheetah_enabled', true );
	...
	static public function get_enabled_templates() {
		$value = self::get_admin_settings_option( '_ba_cheetah_enabled_templates', true );
	...
	static public function is_post_user_template( $type = null ) {
		$post = BACheetahModel::get_post();

		if ( ! $post ) {
			return false;
		} elseif ( 'ba-cheetah-template' == $post->post_type ) {
	...
	static public function save_user_template( $settings = array() ) {
		...
		wp_set_post_terms( $post_id, 'layout', 'ba-cheetah-template-type' );

		// Add category
		$cat       = isset( $settings['category'] ) ? $settings['category'] : '';
		$cat_added = '';
		if ( __( 'Uncategorized', 'ba-cheetah' ) !== $cat && 'uncategorized' !== $cat ) {
			$cat_added = wp_set_object_terms( $post_id, $cat, 'ba-cheetah-template-category' );
		}
		 ...
		 * @see ba_cheetah_after_save_user_template
		 */
		do_action( 'ba_cheetah_after_save_user_template', $post_id );

		$response = array(
			'name'     => $settings['name'],
			'id'       => get_post_meta( $post_id, '_ba_cheetah_template_id', true ),
			'postId'   => $post_id,
			'image'    => BA_CHEETAH_URL . 'img/templates/blank.jpg',
			'kind'     => 'template',
			'content'  => 'layout',
			'type'     => 'user',
			'isGlobal' => false,
			//'link'     => add_query_arg( 'ba_cheetah', '', get_permalink( $post_id ) ),
			'link'     => add_query_arg( 'ba_builder', '', get_permalink( $post_id ) ),
			'category' => array(),
		);
		...
		if ( is_array( $cat_added ) && ! empty( $cat_added ) ) {
			...
			$response['category']['uncategorized'] = __( 'Uncategorized', 'ba-cheetah' );
		}
	...
	static public function get_user_templates( $type = 'layout' ) {
		$categorized = array(
			'uncategorized' => array(
				'name'      => _x( 'Uncategorized', 'Default user template category.', 'ba-cheetah' ),
				'templates' => array(),
			),
		);
		...
		$posts = get_posts( array(
			...
			'tax_query'        => array(
				array(
					'taxonomy' => 'ba-cheetah-template-type',
					'field'    => 'slug',
					'terms'    => $type,
				),
			),
		...
		foreach ( $posts as $post ) {
			...
			$templates[] = array(
				'id'       => get_post_meta( $post->ID, '_ba_cheetah_template_id', true ),
				'postId'   => $post->ID,
				'name'     => $post->post_title,
				'image'    => $image,
				'kind'     => 'template',
				'type'     => 'user',
				'content'  => BACheetahModel::get_user_template_type( $post->ID ),
				'isGlobal' => BACheetahModel::is_post_global_node_template( $post->ID ),
				'link'     => add_query_arg( 'ba_cheetah', '', get_permalink( $post->ID ) ),
				'category' => array(),
			);
		}
		...
		foreach ( $templates as $i => $template ) {
			...
			$cats = wp_get_post_terms( $template['postId'], 'ba-cheetah-template-category' );

			if ( 0 === count( $cats ) || is_wp_error( $cats ) ) {
				$template['category']                        = array(
					'uncategorized' => __( 'Uncategorized', 'ba-cheetah' ),
				);
	...
	static public function get_user_template_type( $template_id = null ) {
		...
		$post = $template_id ? get_post( $template_id ) : BACheetahModel::get_post();

		if( is_object($post) && BACheetahUserTemplatesLayout::is_custom_layout_content_type($post->post_type)) {
			return $post->post_type;
		}
		else if ( ! is_object( $post ) || 'ba-cheetah-template' != $post->post_type ) {
			return '';
		} else {

			$terms = wp_get_post_terms( $post->ID, 'ba-cheetah-template-type' );
	...
	static public function is_node_visible( $node ) {
		...
		return apply_filters( 'ba_cheetah_is_node_visible', $is_visible, $node );
	}
	...
	static public function node_visibility_rules( $node ) {
		...
		switch ( $rule ) {
			...
			case 'logged_in':
				$text = __( 'Logged In', 'ba-cheetah' );
				break;

			case 'logged_out':
				$text = __( 'Logged Out', 'ba-cheetah' );
				break;

			case 'logic':
				$text = __( 'Logic', 'ba-cheetah' );
				break;
		}
	...
	static public function get_node_templates( $type = '' ) {
		...
		foreach ( $posts as $post ) {
			...
			$templates[] = array(
				'id'     => get_post_meta( $post->ID, '_ba_cheetah_template_id', true ),
				'global' => get_post_meta( $post->ID, '_ba_cheetah_template_global', true ),
				'link'   => add_query_arg( 'ba_cheetah', '', get_permalink( $post->ID ) ),
				'name'   => $post->post_title,
			);
		}
	...
	static public function get_node_template_post_id( $template_id ) {
		if ( isset( self::$node_template_post_ids[ $template_id ] ) ) {
			...
			$posts = get_posts( array(
				...
				'posts_per_page' => '-1',
				'post_status'    => 'any',
				'meta_key'       => '_ba_cheetah_template_id',
			...
			$post_id                                      = apply_filters( 'ba_cheetah_node_template_post_id', $posts[0]->ID );
	...
	static public function get_posts_with_global_node_template( $post_id = false ) {
		...
		if ( self::is_post_global_node_template( $post_id ) ) {
			...
			$template_id = get_post_meta( $post_id, '_ba_cheetah_template_id', true );

			$query = new WP_Query( array(
				'meta_query'   => array(
					'relation' => 'OR',
					array(
						'key'     => '_ba_cheetah_data',
						'value'   => $template_id,
						'compare' => 'LIKE',
					),
					array(
						'key'     => '_ba_cheetah_draft',
						'value'   => $template_id,
						'compare' => 'LIKE',
					),
				),
	...
	static public function save_node_template( $template_node_id, $settings ) {
		...
		if ( 'row-inside' == $root_node->type ) {
			$root_node->type = 'row';
		}

		// Set the template type.
		wp_set_post_terms( $post_id, $root_node->type, 'ba-cheetah-template-type' );
		...
		update_post_meta( $post_id, '_ba_cheetah_template_global', $settings['global'] );
		...
		return array(
			...
			'name'   => $settings['name'],
			'type'   => $root_node->type,
			'layout' => $settings['global'] ? BACheetahAJAXLayout::render( $root_node->node, $template_node_id ) : null,
			'config' => $settings['global'] ? BACheetahUISettingsForms::get_node_js_config() : null,
			'postID' => $post_id,
		);
	}
	...
	static public function set_node_template_default_type( $post_id, $post, $update ) {
		...
		if ( $update || 'ba-cheetah-template' != $post->post_type ) {
			return;
		}
		if ( isset( $post_data['ba_cheetah_action'] ) && 'duplicate_post' == $post_data['ba_cheetah_action'] ) {
			return;
		}

		$type = wp_get_post_terms( $post_id, 'ba-cheetah-template-type' );
	...
	static public function unlink_global_node_template_from_all_posts( $template_post_id ) {
		if ( self::is_post_global_node_template( $template_post_id ) ) {
			...
			$template_id = get_post_meta( $template_post_id, '_ba_cheetah_template_id', true );
	...
	static public function delete_global_node_template_from_post( $status, $post_id, $template_id ) {
		...
		foreach ( $layout_data as $node_id => $node ) {
			...
			if ( isset( $node->template_id ) && $node->template_id == $template_id ) {
				...
				uasort( $siblings, array( 'BACheetahModel', 'order_nodes' ) );
	...
	static public function apply_row_inside_template($row_inside_node_id, $module_type, $index) 
	{
		// apply template only if is empty
		if (!empty(self::get_child_nodes($row_inside_node_id))) {
			return;
		}

		$template_file = BA_CHEETAH_DIR."/modules/$module_type/data/template-$index.dat";
		...
		try {
			...
			$template_data = self::get_layout_data_from_file($template_file);
			$template_data = self::generate_new_node_ids( $template_data );

			// Get the root node from the template data.
			$root_node = self::get_node_template_root( 'row', $template_data );

			// remove root element (row)...
			unset($template_data[$root_node->node]);
			foreach ($template_data as $node_id => $value) {
				...
				// ...and update all root-child elements to be child of row-inside
				if ($value->parent == $root_node->node) {
					$template_data[$node_id]->parent = $row_inside_node_id;
				}
			}
			...
			// if something goes wrong, add an empty column
			return self::add_col_group( $row_inside_node_id, '1-col', 0 );
		}
	}
	...
	static public function apply_node_template( $template_id = null, $parent_id = null, $position = 0, $template = null ) {
		 ...
		 * @see ba_cheetah_override_apply_node_template
		 */
		$override = apply_filters( 'ba_cheetah_override_apply_node_template', false, array(
		...
		if ( is_object( $template ) ) {
			...
			$global            = get_post_meta( $template_post_id, '_ba_cheetah_template_global', true );
		}
	...
	static private function register_core_templates() {
		$paths = array(BA_CHEETAH_DIR . 'data/layout-00-Blank-lite.dat');
	 ...
	 * @return void
	 */
	static public function apply_template( $index = 0, $append = false, $content_central_id = 0, $type = 'layout' ) {

		try {
			/**
			 * Allow extensions to hook into applying a template.
			 * @see ba_cheetah_override_apply_template
			 */
			$override = apply_filters( 'ba_cheetah_override_apply_template', false, array(
				'index'  => $index,
				'append' => $append,
				'type'   => $type,
			) );
			...
			return self::apply_core_template( $index, $append, $type, $content_central_id );
	 ...
	 * @return array
	 */
	static public function apply_core_template( $index = 0, $append = false, $type = 'layout', $content_central_id = 0 ) {
		$template     = self::get_template( $index, $type, $content_central_id );
		$row_position = self::next_node_position( 'row' );
		...
		if ( isset( $template->nodes ) ) {
			...
			$template->nodes = BACheetahSettingsCompat::filter_layout_data( $template->nodes );
	 ...
	 * @return object
	 */
	static public function get_template( $index, $type = 'layout', $content_central_id = 0 ) {

		// If the template is pro, get template from content center
		if($content_central_id) {

			// Get templates
			$templates_req = wp_safe_remote_get(BA_CHEETAH_TEMPLATE_API_URL . 'ba-cheetah-template/' . $content_central_id, array(
				'timeout' => 60,
				'headers' => array(
			...
			if (wp_remote_retrieve_response_code($templates_req) === 200) {
				...
				$template = (object) array(
					'id' => $template_data->id,
					'name' => $template_data->title->rendered,
					'category' => isset($template_data->category) ? $template_data->category : $template_data->categories,
					'tags' => array(),
					'type' => $type,
					'kind' => 'template',
					'content' => !in_array($type, array('row', 'column', 'module')) ? 'layout' : $type,
					'pro' => isset($template_data->pro) ? !!$template_data->pro : false,
					'preview_url' => property_exists($template_data, 'preview_url') ? $template_data->preview_url : null,
					'compatible' => property_exists($template_data, 'compatible') ? $template_data->compatible : true,
					'nodes' => maybe_unserialize($template_data->metas->nodes),
					'settings' => array(
						'css' => $template_data->metas->css,
						'js' => $template_data->metas->js
					),
					'global' => false
				);
			...
			throw new BACheetahRetrieveTemplateException(__('Could not retrieve template from content center. Maybe you are without internet, or are you no longer authenticated, or there is a temporary instability on our servers or the template has been removed from the content center. Please try again in a moment.', 'ba-cheetah'), 1);
	...
	static public function get_templates( $type = 'layout', $cached = true ) {
			...
			foreach ( self::$templates as $args ) {
				...
				foreach ( $args['path'] as $path ) {
					...
					if ( stristr( $path, '.php' ) ) {
						...
						$unserialized = ba_cheetah_maybe_fix_unserialize( file_get_contents( $path ) );
					}
		...
		/**
		 * @see ba_cheetah_get_templates
		 */
		return apply_filters( 'ba_cheetah_get_templates', $templates, $type );
	}
	...
	static public function has_templates() {
		return apply_filters( 'ba_cheetah_has_templates', ( count( self::get_templates() ) > 0 ) );
	}
	...
	static public function get_template_selector_data( $type = 'layout' ) {
		$type        = apply_filters( 'ba_cheetah_template_selector_data_type', $type );
		$row_prefix  = 'Rows - ';
		$categorized = array();
		$templates   = array();
		$groups      = array();
		...
		$core_categories = array(
			'general' => __( 'General', 'ba-cheetah' ),
			'landing' => __( 'Landing Pages', 'ba-cheetah' ),
			'company' => __( 'Content Pages', 'ba-cheetah' ),
		);
		...
		foreach ( self::get_templates( $type ) as $key => $template ) {
			...
			// remove 'Rows - ' from rows templates
			if ('row' == $type && strpos($template->group, $row_prefix) === 0) {
				$template->group = substr($template->group, strlen($row_prefix), strlen($template->group));
			}

			if ( strstr( $template->image, '://' ) || strstr( $template->image, ';base64,' ) ) {
				$image = $template->image;
			} else {
				$image = BA_CHEETAH_URL . 'img/templates/' . ( empty( $template->image ) ? 'blank.jpg' : $template->image );
			}

			$templates[] = apply_filters( 'ba_cheetah_template_details', array(
				'id' => $key,
				'content_central_id' => isset($template->id) ? $template->id : null,
				'name' => html_entity_decode($template->name),
				'image' => $image,
				'author' => '',
				...
				'kind' => 'template',
				'content' => ! in_array( $type, array( 'row', 'column', 'module' ) ) ? 'layout' : $type,
				'pro' => isset( $template->pro ) ? ! ! $template->pro : false,
				'preview_url' => property_exists($template, 'preview_url') ? $template->preview_url : null,
				'compatible' => property_exists($template, 'compatible') ? $template->compatible : true
			), $template );
		}
		...
		foreach ( $templates as $i => $template ) {
			...
			if ( ! $template['group'] ) {
				...
				foreach ( $template['category'] as $cat_name ) {
					// remove 'Rows - ' from  free rows templates
					if ('row' == $type && strpos($cat_name, $row_prefix) === 0) {
						$cat_name = substr($cat_name, strlen($row_prefix), strlen($cat_name));
					}
					$template_groups[] = $cat_name;
				}

				/* Commented out to display pro templates when not linked
		 ...
		 * @see ba_cheetah_template_selector_data
		 */
		return apply_filters( 'ba_cheetah_template_selector_data', array(
			'templates'   => $templates,
			'categorized' => $categorized,
			'groups'      => $groups,
		), $type );
	...
	static public function get_row_templates_data() {
		return apply_filters( 'ba_cheetah_row_templates_data', self::get_template_selector_data( 'row' ) );
	}
	...
	static public function get_column_templates_data() {
		return apply_filters( 'ba_cheetah_column_templates_data', self::get_template_selector_data( 'column' ) );
	}
	...
	static public function get_module_templates_data() {
		return apply_filters( 'ba_cheetah_module_templates_data', self::get_template_selector_data( 'module' ) );
	}
	 ...
	 * @return array
	 */
	static public function get_builderall_modules_config() {
		$path   = BA_CHEETAH_DIR . 'json/modules-builderall-config.json';

		return self::get_modules_config($path);
	}
	...
	static public function get_pro_modules_config() {
		$path   = BA_CHEETAH_DIR . 'json/modules-pro-config.json';
	...
	/**
	 * Returns the config for modules if it exists.
	 *

	 * @return array
	 */
	static public function get_modules_config($path) {
		$config = [];
		...
		if ( file_exists( $path ) ) {
			...
			$config = array_map(function($item){
				$item['icon'] = BACheetahModule::get_module_icon($item['icon']);
				return $item;
			}, $contents);
	...
	static public function post_has_pro_modules() 
	{
		$pro_modules = array_keys(BACheetahModel::get_pro_modules_config());
		$post_modules = BACheetahModel::get_nodes('module');

		$pro_modules_count = count(array_filter($post_modules, function($module) use ($pro_modules) {
			return in_array($module->settings->type, $pro_modules);
		}));

		return $pro_modules_count > 0;
	}

	static public function init_color_presets() {
		if (empty(self::get_color_presets())) {
	...
	static public function get_color_presets() {
		return get_option( '_ba_cheetah_color_presets', array() );
	}
	...
	static public function save_color_presets( $presets = array() ) {
		return update_option( '_ba_cheetah_color_presets', $presets );
	}
	...
	static public function is_white_labeled() {
		if ( class_exists( 'BACheetahWhiteLabel' ) ) {
			return BACheetahWhiteLabel::is_white_labeled();
		}
	...
	static public function is_inline_enabled() {
		return apply_filters( 'ba_cheetah_inline_editing_enabled', true );
	}
	...
	static public function is_codechecking_enabled() {
		 ...
		 * @see ba_cheetah_code_checking_enabled
		 */
		$enabled = apply_filters( 'ba_cheetah_code_checking_enabled', true );
		/**
		 * Enable shortcodes in css/js
		 * @see ba_cheetah_enable_shortcode_css_js
	...
	static public function ace_editor_settings() {
		 ...
		 * @see ba_cheetah_ace_editor_settings

		 */
		return apply_filters( 'ba_cheetah_ace_editor_settings', $defaults );
	}
	...
	static public function get_branding_icon() {
		if(BA_CHEETAH_PRO_LIFETIME) {
			return BA_CHEETAH_URL . 'img/branding/logo-always.svg';
		}
		if(BA_CHEETAH_PRO) {
			return BA_CHEETAH_URL . 'img/branding/logo-pro.svg';
		}
		return BA_CHEETAH_URL . 'img/branding/logo.svg';
	}
	...
	static public function get_enabled_icons() {
		$value = self::get_admin_settings_option( '_ba_cheetah_enabled_icons', true );
	...
	static public function get_help_button_settings() {
		if ( class_exists( 'BACheetahWhiteLabel' ) ) {
			return BACheetahWhiteLabel::get_help_button_settings();
		}
	...
	static public function get_row_resize_settings() {
		...
		$settings = apply_filters( 'ba_cheetah_row_resize_settings', $defaults );
	...
	*/
	static public function filter_row_settings_for_resize( $form, $id ) {
		...
		if ( 'row' == $id && ! BACheetahModel::user_can_resize_rows() ) {
	...
	static public function get_services() {
		return get_option( '_ba_cheetah_services', array() );
	}
	...
	static public function update_services( $service, $account, $data ) {
		...
		update_option( '_ba_cheetah_services', $services );
	}
	...
	static public function get_admin_settings_option( $key, $network_override = true ) {
		...
		} elseif ( ! $network_override && class_exists( 'BACheetahMultisiteSettings' ) ) {
			// Get the site-wide option if there's no network override.
			$value = get_site_option( $key );
		} elseif ( class_exists( 'BACheetahMultisiteSettings' ) ) {
	...
	static public function update_admin_settings_option( $key, $value, $network_override = true ) {
		...
		} elseif ( $network_override && BACheetahAdminSettings::multisite_support() && ! isset( $_POST['ba-cheetah-override-ms'] ) ) {
	...
	/**
	 * Returns the plugin basename for Cheetah Builder.
	 *

	 * @return string
	 */
	static public function plugin_basename() {
		return plugin_basename( BA_CHEETAH_DIR . 'ba-cheetah.php' );
	}

	/**
	 * Deletes almost all database data and asset cache for the builder.
	 * We don't delete _ba_cheetah_enabled, _ba_cheetah_data and _ba_cheetah_draft
	...
	static public function uninstall_database() {
		if ( current_user_can( 'delete_plugins' ) ) {
			...
			delete_metadata( 'user', 0, '_ba_cheetah_launched_tour', 1, true );

			// Delete uploaded files and folders.
			$upload_dir = self::get_upload_dir();
			ba_cheetah_filesystem()->rmdir( $upload_dir['path'], true );
	...
	static public function get_theme_branding() {
		_deprecated_function( __METHOD__, '1.8', 'BACheetahWhiteLabel::get_theme_branding()' );

		if ( class_exists( 'BACheetahWhiteLabel' ) ) {
			return BACheetahWhiteLabel::get_theme_branding();
		}
	}
	...
	static public function save_templates( $templates = array() ) {
		_deprecated_function( __METHOD__, '1.8', 'BACheetahCoreTemplatesAdmin::save_templates()' );

		if ( class_exists( 'BACheetahCoreTemplatesAdmin' ) ) {
			BACheetahCoreTemplatesAdmin::save_templates( $templates );
		}
	}
	...
	static public function save_template( $settings ) {
		_deprecated_function( __METHOD__, '1.8', 'BACheetahCoreTemplatesAdmin::save_template()' );

		if ( class_exists( 'BACheetahCoreTemplatesAdmin' ) ) {
			BACheetahCoreTemplatesAdmin::save_template( $settings );
		}
	}
	...
	static public function update_template( $old_index, $settings ) {
		_deprecated_function( __METHOD__, '1.8', 'BACheetahCoreTemplatesAdmin::update_template()' );

		if ( class_exists( 'BACheetahCoreTemplatesAdmin' ) ) {
			BACheetahCoreTemplatesAdmin::update_template( $old_index, $settings );
		}
	}
	...
	static public function delete_template( $index ) {
		_deprecated_function( __METHOD__, '1.8', 'BACheetahCoreTemplatesAdmin::delete_template()' );

		if ( class_exists( 'BACheetahCoreTemplatesAdmin' ) ) {
			BACheetahCoreTemplatesAdmin::delete_template( $index );
		}
	}
	...
	static public function current_user_has_editing_capability() {
		_deprecated_function( __METHOD__, '1.10', 'BACheetahUserAccess::current_user_can()' );

		return BACheetahUserAccess::current_user_can( 'unrestricted_editing' );
	}
	...
	static public function user_templates_admin_enabled() {
		_deprecated_function( __METHOD__, '1.10', 'BACheetahUserAccess::current_user_can( "builder_admin" )' );

		return BACheetahUserAccess::current_user_can( 'builder_admin' );
	}
	...
	/**
	 * If the post is new, set de mode to canvas
	 *
	 * @param  mixed $post_id
	 * @param  mixed $post
	 * @param  mixed $update
	 * @return void
	 */
	static public function save_mode($post_id, $post, $update) {

		$has_meta = get_post_meta($post_id, 'ba_cheetah_mode', true) || get_post_meta($post_id, '_wp_page_template', true);
		$settingsCheetah = get_option( '_ba_cheetah_general_settings' );
		$canvasEnabled  = ($settingsCheetah) ? ($settingsCheetah['canvas-mode'] == 1) ? true : false : true;

		if(!$update && !$has_meta && $post->post_status == 'auto-draft' && $canvasEnabled ) {
			update_post_meta( $post_id, 'ba_cheetah_mode', 'canvas');
			update_post_meta( $post_id, '_wp_page_template', 'ba-cheetah-canvas-template');
		}
