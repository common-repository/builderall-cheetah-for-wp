 ...
 */
final class BACheetahAutoSuggest {
	...
	static public function init() {
		if ( isset( $_REQUEST['ba_cheetah_as_action'] ) && isset( $_REQUEST['ba_cheetah_as_query'] ) ) {

			switch ( $_REQUEST['ba_cheetah_as_action'] ) {

				case 'ba_cheetah_as_posts':
					$data = self::posts();
					break;

				case 'ba_cheetah_as_terms':
					$data = self::terms();
					break;

				case 'ba_cheetah_as_users':
					$data = self::users();
					break;

				case 'ba_cheetah_as_links':
					$data = self::links();
					break;
			}
	...
	static public function get_like() {
		...
		$like = stripslashes( urldecode( $_REQUEST['ba_cheetah_as_query'] ) );
	...
	static public function posts() {
		...
		$data     = array();
		$like     = self::get_like();
		$types    = explode( ',', esc_sql( $_REQUEST['ba_cheetah_as_action_data'] ) );
	...
	static public function terms() {
		...
		$cats = get_categories(array(
			'hide_empty' => 0,
			'taxonomy'   => $_REQUEST['ba_cheetah_as_action_data'],
		));
	...
	static public function links() {
		...
		$data  = array();
		$like  = self::get_like();
		$types = BACheetahLoop::post_types();
