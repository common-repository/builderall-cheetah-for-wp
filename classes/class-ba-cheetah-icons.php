...
final class BACheetahIcons {
	...
	static public function get_sets() {
		...
		if ( is_multisite() ) {
			...
			$enabled_icons = get_option( '_ba_cheetah_enabled_icons' );
		 ...
		 * @see ba_cheetah_icon_sets
		 */
		self::$sets = apply_filters( 'ba_cheetah_icon_sets', self::$sets );
	...
	static public function get_sets_for_current_site() {
		...
		// Get the new sets.
		$sets = apply_filters( 'ba_cheetah_current_site_icon_sets', self::$sets );
	...
	static private function register_core_sets() {
		$enabled_icons = BACheetahModel::get_enabled_icons();
		/**
		 * Array of core icon sets
		 * @see ba_cheetah_core_icon_sets
		 */
		$core_sets = apply_filters( 'ba_cheetah_core_icon_sets', array(
		...
		foreach ( self::$sets as $set_key => $set_data ) {
			if ( 'core' == $set_data['type'] ) {
				...
				$config_path = apply_filters( 'ba_cheetah_core_icon_set_config', BA_CHEETAH_DIR . 'json/' . $key . '.json', $set_data );
	...
	static private function register_custom_sets() {
		// Get uploaded sets.
		$enabled_icons = BACheetahModel::get_enabled_icons();
		$upload_info   = BACheetahModel::get_cache_dir( 'icons' );
		...
		foreach ( $folders as $folder ) {
			...
			if ( ! ba_cheetah_filesystem()->is_dir( $folder ) ) {
				continue;
			}
			...
			if ( ba_cheetah_filesystem()->file_exists( $folder . 'selection.json' ) ) {

				$data = json_decode( ba_cheetah_filesystem()->file_get_contents( $folder . 'selection.json' ) );
				$key  = basename( $folder );
			...
			} elseif ( ba_cheetah_filesystem()->file_exists( $folder . 'config.json' ) ) {

				$data  = json_decode( ba_cheetah_filesystem()->file_get_contents( $folder . 'config.json' ) );
				$key   = basename( $folder );
			...
			} elseif ( ba_cheetah_filesystem()->file_exists( $folder . '/metadata/icons.json' ) ) { // font awesome pro subsets
				$data = json_decode( ba_cheetah_filesystem()->file_get_contents( $folder . '/metadata/icons.json' ) );
				$key  = basename( $folder );
				$url  = str_ireplace( $upload_info['path'], $upload_info['url'], $folder );
	...
	static private function enqueue_styles_for_icon( $icon ) {
		/**
		 * Enqueue the stylesheet for an icon.
		 * @see ba_cheetah_enqueue_styles_for_icon
		 */
		do_action( 'ba_cheetah_enqueue_styles_for_icon', $icon );
	...
	static private function enqueue_custom_styles_by_key( $key ) {
		if ( apply_filters( 'ba_cheetah_enqueue_custom_styles_by_key', true, $key ) ) {
			$sets = self::get_sets();
			...
			if ( isset( $sets[ $key ] ) ) {
				...
				if ( 'icomoon' == $set['type'] ) {
					wp_enqueue_style( $key, $set['stylesheet'], array(), BA_CHEETAH_VERSION );
				}
